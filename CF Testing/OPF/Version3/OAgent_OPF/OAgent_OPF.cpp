...
bool OAgent_OPF::AcceleratedOPF(bool genBus) {
    ...
    for(uint16_t k = 0; k < iterations; k++)
    {
        ...
        if (k%100==0){
            Serial << "Iteration " << k+1<<endl;
            _print_("P=",P,6);_print_("Q=",Q,6);_print_("V=",V,6);_print_("bP=",bP,6);_print_("bQ=",bQ,6);
         //    for (uint8_t i:neighbors) {
         //    	_print_("Voltage drop constraint ", bn[i-1],6);
        	// }
            delay(5);
        }
        ...
        if (k%comm==0){
            ...
            while (millis()-start<=500)
            {
                ...
                {
                    float vars[6] = {comm_self_fp[nei_to_send-1],comm_self_fq[nei_to_send-1],comm_neighbor_fp[nei_to_send-1],comm_neighbor_fq[nei_to_send-1],comm_neighbor_V[nei_to_send-1],comm_self_V[nei_to_send-1]};
                ...
                {
                    float vars[4] = {fp[nei_to_send-1],fq[nei_to_send-1],comm_neighbor_V[nei_to_send-1],comm_self_V[nei_to_send-1]};
                ...
                if(_waitForUnicastPacket(neighborID,nodeID,OPF_HEADER,true,100))                                // OPF packet available for node from its neighbor
                {
                    ...
                    if (!received_from[neighborID-1])
                    {
                        ...
                        if(neighborID < nodeID)  // in-coming flow, nodeID - child
                        {
                            ...
                            neighbor_fp = tmp[0]; neighbor_yp = tmp[4];
                            neighbor_fq = tmp[1]; neighbor_yq = tmp[5];
                            self_V = tmp[2]; neighbor_V = tmp[3];
                            ...
                            {    
                                ...
                                received_from_parents[neighborID-1]=1;
                                ...
                                received_self_V[neighborID-1] = self_V;
                                received_neighbor_V[neighborID-1] = neighbor_V;
                        ...
                        else if((neighborID > nodeID))  //out-going flow, nodeID - parent
                        {
                            ...
                            self_V = tmp[4]; neighbor_V = tmp[5];                      
                            neighbor_old_yp = tmp[6]; neighbor_old_yq = tmp[7];
                            self_old_yp = tmp[8]; self_old_yq = tmp[9];
        ...
        uint8_t count=1;
        for (uint8_t i:neighbors) {
            if (received_from_parents[i-1]){  
                count+=1;
                tot+=received_self_V[i-1];
                if (k>=1400 and k%50==0) Serial<<"Node "<<i<<endl;
                // Serial<<"Parent " <<i<<" voltage "<<parent_V[i-1]<<" "<<parent_V[i-1] - 2*((n+(i-1))->getResistance())*new_fp[i-1] - 2*((n+(i-1))->getReactance())*new_fq[i-1]<<endl;
            }
            if (received_from_children[i-1]){  
                count+=1;
                tot+=received_self_V[i-1];
                if (k>=1500 and k%50==0) Serial<<"Node "<<i<<endl;
                // Serial<<"Parent " <<i<<" voltage "<<parent_V[i-1]<<" "<<parent_V[i-1] - 2*((n+(i-1))->getResistance())*new_fp[i-1] - 2*((n+(i-1))->getReactance())*new_fq[i-1]<<endl;
            }
            received_from_parents[i-1]=0; received_from_children[i-1]=0;
        }
        // Serial<<"Parents "<<count<<endl;
        new_V = V + count*0.2*(tot/count-V); 
        if (k%100==0){
        	for (uint8_t i:neighbors) {
	            if (i>nodeID){
	            	// Serial<<"Neighbor "<<i<<endl; delay(5);
	            	// _print_("Self voltage ",new_V,6);
	            	// _print_("Neighbor voltage ",received_neighbor_V[i-1],6);
	                bn[i-1] = new_V - received_neighbor_V[i-1] - 2*((n+(i-1))->getResistance())*new_fp[i-1] - 2*((n+(i-1))->getReactance())*new_fq[i-1];
	                _print_("Voltage drop constraint ",bn[i-1],6);
	            }
	            else{
	            //    	Serial<<"Neighbor "<<i<<endl; delay(5);
        	...
        	_print_("Voltage imbalance ",tot/count-V,6);
        } //_print_("Voltage imbalance ",tot/count-V,6);
        ...
        for (uint8_t i:neighbors) {
            ...
            if (!received_from[i-1]){
                ...
                if (i>nodeID){ 
                    // if (adjust){
                    new_fp[i-1] += zeta*((n+(i-1))->getResistance())*max(0,Vmin-new_V) - zeta*((n+(i-1))->getResistance())*max(0,new_V-Vmax);
                    new_fq[i-1] += zeta*((n+(i-1))->getReactance())*max(0,Vmin-new_V) - zeta*((n+(i-1))->getResistance())*max(0,new_V-Vmax); 
        ...
        for (uint8_t i:neighbors) {
            if (i>nodeID){
                ...
                // new_yp[i-1] += zeta*((n+(i-1))->getResistance())*max(0,Vmin-new_V) - 2*((n+(i-1))->getResistance())*max(0,new_V-Vmax);
                // new_yq[i-1] += zeta*((n+(i-1))->getReactance())*max(0,Vmin-new_V) - 2*((n+(i-1))->getResistance())*max(0,new_V-Vmax);
                comm_neighbor_V[i-1] = V - 2*((n+(i-1))->getResistance())*new_fp[i-1] - 2*((n+(i-1))->getReactance())*new_fq[i-1];
                comm_self_V[i-1] = V;
            }
            else{
                new_yp[i-1] += 2*(new_lambda+beta1*new_bP-lambda-beta1*bP);
    ...
    for (uint8_t i:neighbors) {
    		Serial<<"Node "<<i<<endl;
            _print_("Voltage drop constraint ", bn[i-1],6);
            _print_("Active flow ", fp[i-1],6);
            _print_("Reactive flow ", fq[i-1],6);
