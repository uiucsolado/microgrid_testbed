...
bool OAgent_OPF::AcceleratedOPF(bool genBus) {
    ...
    float bn[NUM_REMOTE_VERTICES]; for (int i=0;i<NUM_REMOTE_VERTICES;i++)bn[i]=0;
     ...
     /// Control Parameters
    float alpha=0.1,beta1=0.3,beta2=0.3,zeta=0.1,alpha_V=1,iterations=500; uint8_t comm=5;
    ...
    for(uint16_t k = 0; k < iterations; k++)
    {
        ...
        if (k%comm==0){
            ...
            while (millis()-start<=500)
            {
                ...
                {
                    float vars[6] = {comm_self_fp[nei_to_send-1],comm_self_fq[nei_to_send-1],comm_neighbor_fp[nei_to_send-1],comm_neighbor_fq[nei_to_send-1],comm_V[nei_to_send-1],comm_adjust_V[nei_to_send-1]};
                ...
                {
                    float vars[3] = {fp[nei_to_send-1],fq[nei_to_send-1],comm_V[nei_to_send-1]};
                ...
                if(_waitForUnicastPacket(neighborID,nodeID,OPF_HEADER,true,100))                                // OPF packet available for node from its neighbor
                {
                    ...
                    if (!received_from[neighborID-1])
                    {
                        ...
                        if(neighborID < nodeID)  // in-coming flow, nodeID - child
                        {
                            ...
                            neighbor_fp = tmp[0]; neighbor_yp = tmp[3];
                            neighbor_fq = tmp[1]; neighbor_yq = tmp[4];
                            neighbor_V = tmp[2];
                            ...
                            {    
                                ...
                                parent_V[neighborID-1] = neighbor_V;
                        ...
                        else if((neighborID > nodeID))  //out-going flow, nodeID - parent
                        {
                            ...
                            neighbor_V = tmp[4]; change_V = tmp[5];                      
                            ...
                            {       
                                ...
                                child_V[neighborID-1] = neighbor_V;
                                adjust_V[neighborID-1] = change_V;
        ...
        uint8_t count=0;
         for (uint8_t i:neighbors) {
            if (received_from_parents[i-1]){  
                count+=1;
                average_v+=parent_V[i-1] - 2*((n+(i-1))->getResistance())*new_fp[i-1] - 2*((n+(i-1))->getReactance())*new_fq[i-1];
                // Serial<<"Parent " <<i<<" voltage "<<parent_V[i-1]<<" "<<parent_V[i-1] - 2*((n+(i-1))->getResistance())*new_fp[i-1] - 2*((n+(i-1))->getReactance())*new_fq[i-1]<<endl;
            }
            received_from_parents[i-1]=0;
        }
        // Serial<<"Parents "<<count<<endl;
        if (count) new_V = average_v/count; 
        else new_V = V;
        float max_V=new_V, min_V=new_V;
        for (uint8_t i:neighbors) {
            if (received_from_children[i-1]){
                if (adjust_V[i-1]>0){
                    max_V = max(max_V,child_V[i-1]+adjust_V[i-1]); adjust = true;
                }
                else if (adjust_V[i-1]<0){
                    min_V = min(min_V,child_V[i-1]+adjust_V[i-1]); adjust = true;
                }
                // Serial<<"Child "<<i<<" voltage "<<child_V[i-1]<<" "<<adjust_V[i-1]<<endl;
                adjust_V[i-1] = 0;
            }
            received_from_children[i-1]=0;
        }
        new_V = 0.5*(max_V+min_V); //new_V = 1;
        // Serial<<"Voltage "<<new_V<<endl;
        ...
        for (uint8_t i:neighbors) {
            if (i>nodeID){
                ...
                // new_yp[i-1] -= zeta*((n+(i-1))->getResistance())*max(0,Vmin-new_V) - 2*((n+(i-1))->getResistance())*max(0,new_V-Vmax);
                // new_yq[i-1] -= zeta*((n+(i-1))->getReactance())*max(0,Vmin-new_V) - 2*((n+(i-1))->getResistance())*max(0,new_V-Vmax);
                comm_V[i-1] = parent_V[i-1]; comm_adjust_V[i-1] = -(min(0,new_V-Vmin) + max(0,new_V-Vmax));
            } 
...
void OAgent_OPF::_SendToChild(uint16_t recipientID, bool flag_OPF, float* vars, float* grad) {
    ...
    for (int i=5,j=0;i<20,j<3;i=i+5,j++){
    ...
    for (int i=20,j=0;i<30,j<2;i=i+5,j++){
