...
bool OAgent_OPF::AcceleratedOPF(bool genBus) {
    ...
    for(uint16_t k = 0; k < iterations; k++)
    {
        ...
        // for (uint8_t i:neighbors) {
        //     _print_("Active flow ", fp[i-1],6); _print_("Reactive flow ", fq[i-1],6); _print_("Active flow grad ", yp[i-1],6); _print_("Reactive flow grad ", yq[i-1],6); delay(5);
        // }
        ...
        if (k%comm==0){
            ...
            while (millis()-start<=500)
            {
                ...
                if (nei_to_send<nodeID) 
                {
                    ...
                    // float vars_c[5] = {comm_self_fp[nei_to_send-1],comm_self_fq[nei_to_send-1],comm_neighbor_fp[nei_to_send-1],comm_neighbor_fq[nei_to_send-1],comm_self_lmbd_v[nei_to_send-1]};
                    // float grad_c[5] = {comm_self_yp[nei_to_send-1],comm_self_yq[nei_to_send-1],comm_neighbor_yp[nei_to_send-1],comm_neighbor_yq[nei_to_send-1],comm_self_ylmbd[nei_to_send-1]};
                    // _SendToParent(nei_to_send,self_flags[nei_to_send-1],vars_c,grad_c,1);
                }
                    ...
                    if (!received_from[neighborID-1])
                    {
                        ...
                        if(neighborID < nodeID)  // in-coming flow, nodeID - child
                        {
                            ...
                            {    
                                ...
                                // Serial<<"Info received from node "<<neighborID<<endl;
                                // _print_("Active flow ", neighbor_fp,6); _print_("Active flow grad ", neighbor_yp,6); delay(5);
                            ...
                            if (neighbor_flags[neighborID-1] != neighbor_flag){ 
                                ...
                                if (!(packet!=received_packet_num[neighborID-1] && received_packet_num[neighborID-1]>0)) {
                                    if (received_packet_num[neighborID-1]==0){
                                        received_packet_num[neighborID-1] = packet; //Serial<<"Node "<<neighborID<<"Packet "<<packet<<endl; delay(5);
                                        for (int t=0;t<10;t++) received_packets[packet-1][t]=tmp[t];
                                    }
                                }
                                else 
                                {
                                    // neighbor_old_fp = tmp[0]; neighbor_old_yp = tmp[5];self_old_fp = tmp[2]; self_old_yp = tmp[7]; 
                                    // neighbor_old_fq = tmp[1]; neighbor_old_yq = tmp[6];self_old_fq = tmp[3]; self_old_yq = tmp[8];
                                    
                                    // _print_("Active flow ", neighbor_old_fp,6); _print_("Active flow ", self_old_fp,6); _print_("Active flow grad ", neighbor_old_yp,6); _print_("Active flow grad ", self_old_yp,6); delay(5);
                                    received_from[neighborID-1]=1; //Serial<<"Node "<<neighborID<<"Packet "<<packet<<endl; delay(5);
                                    for (int t=0;t<10;t++) received_packets[packet-1][t]=tmp[t];
        ...
        for (uint8_t i:neighbors) {
            ...
            if (!received_from[i-1]){
                ...
                new_nu[i-1] = nu[i-1] + alpha*yn[i-1];
                new_yn[i-1] = yn[i-1];
        ...
        new_V = _clip(V - alpha*dV,Vmin,Vmax);
...
void OAgent_OPF::_SendToChild(uint16_t recipientID, bool flag_OPF, float* vars, float* grad) {
    ...
    for (int i=30,j=0;i<55,j<5;i=i+5,j++){
        ...
        // Serial<<"Information to be sent "<<endl;
        // Serial<<grad[j]<<" "<<sign_var<<" "<<var<<endl;
        payload[i]=sign_var;
        payload[i+1] = var;
        payload[i+2] = var >> 8;
        payload[i+3] = var >> 16;
...
void OAgent_OPF::_SendToParent(uint16_t recipientID, bool flag_OPF, float* vars, float* grad, uint8_t packet){
    uint8_t payload[56];
...
float* OAgent_OPF::_getPacketFromChild() {
    ...
    for (int i=5,j=0;i<55,j<10;i=i+5,j++){
        ...
        // if (j==5 or j==7) {Serial<<"Active flow grad "<<endl; Serial<<x<<" "<<mag_x<<endl;delay(5);}
        a[j] = x/BASE;
...
uint8_t OAgent_OPF::_whichPacket() {
    uint8_t packet = _rx->getData(55);
