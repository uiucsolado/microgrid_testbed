...
float* OAgent_OPF::Conjugate_gradient(float*A, int rows, int cols, float*b, float*x_init) {
    ...
    for (int k=0;k<10;k++)
    {
        ...
        while (millis()-start<=10000){
            ...
            if (nei_to_send<nodeID)_SendToParent(nei_to_send,vars,16,1);
            else _SendToChild(nei_to_send,vars,16,1);
            if(_waitForUnicastPacket(neighborID,nodeID,OPF_HEADER,true,150))
            {
            	int j = nei2index[neighborID-1];
            	if (!received_from[j])
                {	 
                	if (neighborID>nodeID)
                	{
                		float *tmp=_getPacketFromChild(16);
                		uint8_t task = tmp[16];
	                    if (task==1)
	                    {
	                        received_from[j]=1;
                	...
                	{
                		float *tmp=_getPacketFromParent(16);
                		uint8_t task = tmp[16];
	                    if (task==1)
	                    {
	                        received_from[j]=1;	
     ...
     //    str="x"; _print1Darray_(str,x,4*deg,6);
        ...
        nu = 1;
        float alpha3 = RunRatioConsensus(nodeID,&mu1,&nu,50,neighbors);
        // str="alpha3";_print_(str,alpha3,6); 

        float beta = alpha3/alpha2;
