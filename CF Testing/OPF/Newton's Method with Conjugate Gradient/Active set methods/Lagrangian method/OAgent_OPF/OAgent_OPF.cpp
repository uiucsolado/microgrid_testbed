...
bool OAgent_OPF::SecondOrderOPF(bool genBus) {
    ...
    Serial<<Pmax<<" "<<Pmin<<" "<<Qmax<<" "<<Qmin<<" "<<Vmax<<" "<<Vmin<<endl; 
    ...
    // P,Q,V,p,q
    float A[5+deg][3+2*deg];
    for (int i=0;i<5+deg;i++){
    ...
    float A_T[3+2*deg][5+deg];
    MatrixTranspose(&A_T[0][0],&A[0][0],5+deg,3+2*deg);
    float L[5+deg][5+deg];
    for (int i=0;i<5+deg;i++) 
    {
    	for (int j=0;j<5+deg;j++){L[i][j]=0;}
    }
	L[0][0] = 1+deg; 
    for (int i=0;i<deg;i++) {
    ...
    float a[3+2*deg],b[5+deg];
    for (int i=0;i<3+2*deg;i++) 
    {
        a[i]=0.0;
        if (i==2) a[i]=1.0;
    }
    for (int i=0;i<5+deg;i++) b[i]=0;
    b[0]=Pd; b[1] = Qd;

    float *x = new float[3+2*deg];
    float *w = new float[5+deg]; for (int i=0;i<5+deg;i++)w[i]=0;
    float *y = new float[5+deg];
    String str;
    float *r = new float[5+deg]; float *new_r = new float[5+deg];

    for (int k=0;k<3;k++)
    {
    	// str="A";_print2Darray_(str,&A[0][0],5+deg,3+2*deg,3);
	    // str="A_T";_print2Darray_(str,&A_T[0][0],3+2*deg,5+deg,3);
	    // str="L";_print2Darray_(str,&L[0][0],5+deg,5+deg,3);

    	int factor = 1;
	    y = matbyvec(&A[0][0],5+deg,3+2*deg,a);
	    for (int i=0;i<5+deg;i++) y[i]-=b[i];
		// str = "y";_print1Darray_(str,y,5+deg,6);
		uint8_t round = k;
	    Conjugate_gradient(&L[0][0],y,w,factor,round);
	    // str = "w";_print1Darray_(str,w,5+deg,6);

	    // r = matbyvec(&L[0][0],5+deg,5+deg,w);
	    // for (int i=0;i<5+deg;i++) r[i]=r[i]-y[i];
	    // for (int i=0;i<5+deg;i++) new_r[i]=r[i];
	    // str = "r";_print1Darray_(str,r,5+deg,6); 
	    ...
	    // }
	    // for (int i=0;i<5+deg;i++) r[i]=new_r[i];
	    // str = "r";_print1Darray_(str,r,5+deg,6);

		x = matbyvec(&A_T[0][0],3+2*deg,5+deg,w);
	    ...
	    // float *r = new float[5+deg]; float *new_r = new float[5+deg];
	    // r = matbyvec(&A[0][0],5+deg,3+2*deg,x);
	    // for (int i=0;i<5+deg;i++) r[i]=r[i]-b[i];
	    // for (int i=0;i<5+deg;i++) new_r[i]=r[i];
	    // str = "r";_print1Darray_(str,r,5+deg,6); 
	    ...
	    // }
	    // for (int i=0;i<5+deg;i++) r[i]=new_r[i];
	    // _print1Darray_(str,r,5+deg,6);
		if (x[0]>Pmax or x[0]<Pmin) 
		{
			L[2+deg][0] = 1;L[2+deg][2+deg] = 1;L[0][2+deg] = 1;
			if (x[0]>Pmax) b[2+deg] = Pmax;
			else b[2+deg] = Pmin;
			A[2+deg][0] = 1;
			// Serial<<"Active power injection: "<<x[0]<<endl;
		}
		if (x[1]>Qmax or x[1]<Qmin) 
		{
			L[3+deg][1] = 1;L[3+deg][3+deg] = 1;L[1][3+deg] = 1;
			if (x[1]>Qmax) b[3+deg] = Qmax;
			else b[3+deg] = Qmin;
			A[3+deg][1] = 1;
			// Serial<<"Reactive power injection: "<<x[1]<<endl;
		}
		if (x[2]>Vmax or x[2]<Vmin) 
		{
			L[4+deg][4+deg] = 1;
			if (x[2]>Vmax) b[4+deg] = Vmax;
			else b[4+deg] = Vmin;
		    for (int i=0;i<deg;i++) {
		        if (neighbors[i]>nodeID) {L[4+deg][2+i]=1;L[2+i][4+deg]=1;}
		        else {L[4+deg][2+i]=-1;L[2+i][4+deg]=-1;}
		    }
		    A[4+deg][2] = 1;
		    // Serial<<"Voltage: "<<x[2]<<endl;
		}
		MatrixTranspose(&A_T[0][0],&A[0][0],5+deg,3+2*deg);
...
void OAgent_OPF::Conjugate_gradient(float*A, float*b, float*x, int &factor, uint8_t &round) {
    ...
    float *r = new float[5+deg];
    // float *x = new float[5+deg]; for (int i=0;i<5+deg;i++) x[i]=w[i];

    // r = matbyvec(&H[0][0],5+deg,5+deg,x);
    r = matbyvec(A,5+deg,5+deg,x);
    for (int i=0;i<5+deg;i++) r[i]=r[i]-b[i];
    float *new_r = new float[5+deg]; for (int i=0;i<5+deg;i++) new_r[i]=r[i];
    String str = "r";
    // _print1Darray_(str,r,5+deg,6); 
    ...
    for (int i=0;i<5+deg;i++) r[i]=new_r[i];
    // _print1Darray_(str,r,5+deg,6);
    float *p=new float[5+deg]; for (int i=0;i<5+deg;i++) p[i]=-r[i];
    float *q=new float[5+deg]; for (int i=0;i<5+deg;i++) q[i]=0;
    float *new_q = new float[5+deg];
    float *new_p = new float[5+deg]; 
    float *new_x = new float[5+deg];

    float mu1=r[0]*r[0]+r[1]*r[1]+r[2+deg]*r[2+deg]+r[3+deg]*r[3+deg]+r[4+deg]*r[4+deg];
    ...
    for (int k=0;k<15;k++)
    {
    	...
    	for (int i=0;i<deg;i++) received_from[i]=0;
        // q = matbyvec(&H[0][0],5+deg,5+deg,p);
        // r = matbyvec(&H[0][0],5+deg,5+deg,x);
    	q = matbyvec(A,5+deg,5+deg,p);
        r = matbyvec(A,5+deg,5+deg,x);
        for (int i=0;i<5+deg;i++) r[i]=r[i]-b[i];
        // str = "q";_print1Darray_(str,q,5+deg,6);
        for (int i=0;i<5+deg;i++) {new_q[i]=q[i];new_r[i]=r[i];new_x[i]=x[i];new_p[i]=p[i];}
        startTime = myMillis()+SCHEDULE_OPF_DELAY;
	    scheduled = false;
	    while (!scheduled)
	    { 
        ...
        for (int i=0;i<deg;i++) received_from[i]=0;
        for (int i=0;i<5+deg;i++) {q[i]=new_q[i];r[i]=new_r[i];p[i]=new_p[i];x[i]=new_x[i];}   
        // str = "q";_print1Darray_(str,q,5+deg,6);

        float mu2=p[0]*q[0]+p[1]*q[1]+p[2+deg]*q[2+deg]+p[3+deg]*q[3+deg]+p[4+deg]*q[4+deg];
        ...
        // str="r"; _print1Darray_(str,r,5+deg,6);
        mu1=r[0]*r[0]+r[1]*r[1]+r[2+deg]*r[2+deg]+r[3+deg]*r[3+deg]+r[4+deg]*r[4+deg];
	    for (int i=2;i<2+deg;i++){mu1 += 0.5*r[i]*r[i];}
        mu1 = float(mu1*factor);
        ...
        // str="r"; _print1Darray_(str,r,5+deg,6);
        // str="p"; _print1Darray_(str,p,5+deg,6);
        // str="x"; _print1Darray_(str,x,5+deg,6);

    }

 	// str="Newton direction";_print1Darray_(str,x,5+deg,5);
