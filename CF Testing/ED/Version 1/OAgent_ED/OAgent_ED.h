...
#define SCHEDULE_ED_HEADER               0x7340 // schedule coordinate header is ascii s@
#define SCHEDULE_FAIR_SPLIT_HEADER       0x7346 // schedule coordinate header is ascii sF
#define SCHEDULE_ED_ACK_HEADER           0x7334 // schedule coordinate header is ascii s4
#define SCHEDULE_ED_ACKACK_HEADER        0x7500 // schedule coordinate header is ascii s4
...
#define ED_ACK_HEADER                    0x6B50 // Primal-dual acknowledgment header is kP
...
class OAgent_ED {
	public:
        ...
        /// Methods
        inline OGraph_ED * getGraph() { return _G; }
        ...
        // Primal Dual methods
        bool EconomicDispatch(bool genBus, float alpha, uint8_t iterations);
        bool leaderED(bool genBus, float alpha, uint8_t iterations);
        bool nonleaderED(bool genBus, float alpha, uint8_t iterations);
        bool StandardED(bool genBus);
        bool AcceleratedED(bool genBus);
        float _clip(float x, float xmin, float xmax);
        void _sender_helper(float x,uint8_t* sign_y,uint32_t* y);
        void _SendPacket(uint16_t recipientID, float* vars);
        float* _getPacket();
	...
	private:
        ...
        inline void _broadcastSchedulePrimalDualPacket(unsigned long startTime, uint8_t iterations, uint16_t period) {
            _broadcastSchedulePacket(SCHEDULE_ED_HEADER,startTime,iterations,period);
        }
        ...
        //inline bool _EDPacketAvailable() { return _packetAvailable(ED_PACKET_HEADER,true); }
        ...
        void _unicastPacket_ED_P(uint16_t recipientID, float fP, bool flag_ED);
        void _unicastPacket_ED_C(uint16_t recipientID, float fP_c, bool flag_ED, float fP_p);
        ...
        bool _waitForSchedulePacketED(int timeout, unsigned long startTime, uint8_t iterations);
        ...
        void _broadcastSchedulePacketED(unsigned long startTime, uint8_t numIterations);
