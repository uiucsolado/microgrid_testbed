...
OGraph_SCED::OGraph_SCED(OLocalVertex * s) {
    _n = 1;
    _self = s;
    //linkedlist for online neighbors
    LinkedList _l = LinkedList();
...
OGraph_SCED::OGraph_SCED(OLocalVertex * s, LinkedList * l) {
    _n = 1;
    _self = s;
    //linkedlist for online neighbors
    _list = l;
...
// Local vertex
//OLocalVertex * OGraph_SCED::getLocalVertex() {
//    return _self;
//}

bool OGraph_SCED::isLocalVertex(uint32_t aLsb) {
...
//bool OGraph_SCED::isLocalVertex(XBeeAddress64 a) {
//	return isLocalVertex(a.getLsb());
//}

// Graph properties
//uint8_t OGraph_SCED::getN() {
//    return _n;
//}

//uint8_t OGraph_SCED::getNumberNonNeighbors() {
//    return _n - _self->getInDegree() - 1;
//}

// Add and remove vertices from in-neighborhood
bool OGraph_SCED::addInNeighbor(XBeeAddress64 a, uint8_t neighborID) {
    return addInNeighbor(a.getLsb(),neighborID);
}

bool OGraph_SCED::addInNeighbor(uint32_t aLsb, uint8_t neighborID) {
    return addInNeighbor(aLsb,neighborID,0,0);
}

bool OGraph_SCED::addInNeighbor(XBeeAddress64 a, uint8_t neighborID, float r, float x) {
    return addInNeighbor(a.getLsb(),neighborID,r,x);
}

bool OGraph_SCED::addInNeighbor(uint32_t aLsb, uint8_t neighborID, float r, float x) {
...
bool OGraph_SCED::removeInNeighbor(uint8_t neighborID){
...
bool OGraph_SCED::isInNeighbor(XBeeAddress64 a) {
    return isInNeighbor(a.getLsb());
}

bool OGraph_SCED::isInNeighbor(uint32_t aLsb) {
    uint8_t i;
    return isInNeighbor(aLsb,i);
}

bool OGraph_SCED::isInNeighbor(XBeeAddress64 a, uint8_t &i) {
    return isInNeighbor(a.getLsb(),i);
}

bool OGraph_SCED::isInNeighbor(uint32_t aLsb, uint8_t &i) {
    if(_isRemoteVertex(aLsb,i))
        return _remoteVertices[i].isInNeighbor();
    return false;
}

bool OGraph_SCED::isInNeighbor(uint32_t aLsb, ORemoteVertex * &v) {
    uint8_t i;
...
bool OGraph_SCED::addNonNeighbor(XBeeAddress64 a, uint8_t neighborID) {
    return addNonNeighbor(a.getLsb(),neighborID);
}

bool OGraph_SCED::addNonNeighbor(uint32_t aLsb, uint8_t neighborID) {
    return addNonNeighbor(aLsb,neighborID,0,0);
}

bool OGraph_SCED::addNonNeighbor(XBeeAddress64 a, uint8_t neighborID, float r, float x) {
    return addNonNeighbor(a.getLsb(),neighborID,r,x);
}

bool OGraph_SCED::addNonNeighbor(uint32_t aLsb, uint8_t neighborID, float r, float x) {
    return _addRemoteVertex(aLsb,neighborID,r,x);
}

// Determine if vertex is non-neighbor
bool OGraph_SCED::isNonNeighbor(XBeeAddress64 a) {
    return isNonNeighbor(a.getLsb());
}

bool OGraph_SCED::isNonNeighbor(uint32_t aLsb) {
    uint8_t i;
    return isNonNeighbor(aLsb,i);
}

bool OGraph_SCED::isNonNeighbor(XBeeAddress64 a, uint8_t &i) {
    return isNonNeighbor(a.getLsb(),i);
}

bool OGraph_SCED::isNonNeighbor(uint32_t aLsb, uint8_t &i) {
    if(_isRemoteVertex(aLsb,i))
        return !(_remoteVertices[i].isInNeighbor());
    return false;
}
...
void OGraph_SCED::clearAllNuTau() {
	_self->clearAllTau();
...
void OGraph_SCED::clearAllYMinYMax() {
    _self->clearYMinYMax();
	for(uint8_t i = 0; i < NUM_REMOTE_VERTICES; i++)
		_remoteVertices[i].clearYMinYMax();
}

void OGraph_SCED::clearAllInStates() {
    _self->clearZIn();
...
void OGraph_SCED::clearAllBroadcastStates() {
	_self->clearSigma();
...
void OGraph_SCED::clearAllStates() {
    _self->clearZInSigma();
    _self->setZ(0);
    _self->clearAllTau();
    _self->clearYMinYMax();
...
OVertex * OGraph_SCED::getVertexByAddressLsb(uint32_t aLsb) {
    uint8_t i;
    return getVertexByAddressLsb(aLsb,i);
}

// Methods to pointer to vertex
OVertex * OGraph_SCED::getVertexByAddressLsb(uint32_t aLsb, uint8_t &i) {
...
OVertex * OGraph_SCED::getVertexByAddress(XBeeAddress64 a) {
	return getVertexByAddressLsb(a.getLsb());
}

OVertex * OGraph_SCED::getVertexByUniqueID(uint8_t i) {
	if(i == 0) {
		return dynamic_cast<OVertex*> (_self);
...
ORemoteVertex * OGraph_SCED::getRemoteVertex(uint8_t neighborID) {
...
void OGraph_SCED::configureLinkedList() {
    _list->updateLinkedList(_self->getStatusP());
}

uint8_t OGraph_SCED::_getRemoteVertexIndex(XBeeAddress64 a) {
    return _getRemoteVertexIndex(a.getLsb());
}

uint8_t OGraph_SCED::_getRemoteVertexIndex(uint32_t aLsb) {
	for(uint8_t i = 0; i < NUM_REMOTE_VERTICES; i++) {
...
bool OGraph_SCED::_isRemoteVertex(uint32_t aLsb) {
    uint8_t i = _getRemoteVertexIndex(aLsb);
    return _isRemoteVertex(aLsb,i);
}

bool OGraph_SCED::_isRemoteVertex(uint32_t aLsb, uint8_t &i) {
...
bool OGraph_SCED::_addRemoteVertex(uint32_t aLsb, uint8_t neighborID, float r, float x, bool inNeighbor) {
