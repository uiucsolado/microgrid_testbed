...
#ifndef OAgent_PDRC_h
#define OAgent_PDRC_h
...
class OAgent_PDRC {
	public:
        /// Constructors
		OAgent_PDRC();
        OAgent_PDRC(XBee * xbee, OGraph_PDRC * G, bool leader = false, bool quiet = true);
        OAgent_PDRC(XBee * xbee, ZBRxResponse * rx, OGraph_PDRC * G, bool leader = false, bool quiet = true);
        //OAgent_PDRC(XBee * xbee, ZBRxResponse * rx, OGraph_PDRC * G, bool leader = false, bool quiet = true, int RS = 0);
        
        /// Methods
        inline OGraph_PDRC * getGraph() { return _G; }
	...
	private:
        ...
        //inline bool _PDRCPacketAvailable() { return _packetAvailable(PD_PACKET_HEADER,true); }
        ...
        void _unicastPacket_PDRC_P(uint16_t recipientID, float fP, float fQ, float Lambda, bool flag);
        void _unicastPacket_PDRC_C(uint16_t recipientID, float fP_c, float fQ_c, float Lambda_c, bool flag, float fP_p, float fQ_p, float Lambda_p);
        ...
        void _prepareOAgent_PDRC(XBee * xbee, ZBRxResponse * rx, OGraph_PDRC * G, bool leader = false, bool quiet = true);
};
