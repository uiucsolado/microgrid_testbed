...
//// Public methods
/// Constructors
OAgent_DFC::OAgent_DFC() {

    XBee temp1 = XBee();
    ZBRxResponse temp2 = ZBRxResponse();
    OGraph_DFC temp3 = OGraph_DFC();
    _prepareOAgent_DFC(&temp1,&temp2,&temp3);
    //setRS(0);
}

OAgent_DFC::OAgent_DFC(XBee * xbee, OGraph_DFC * G, bool leader, bool quiet) {
     ZBRxResponse temp = ZBRxResponse();
    _prepareOAgent_DFC(xbee,&temp,G,leader,quiet);
    //setRS(0);
}

OAgent_DFC::OAgent_DFC(XBee * xbee, ZBRxResponse * rx, OGraph_DFC * G, bool leader, bool quiet) {
    _prepareOAgent_DFC(xbee,rx,G,leader,quiet);
    //setRS(0);
}

// OAgent_DFC::OAgent_DFC(XBee * xbee, ZBRxResponse * rx, OGraph_DFC * G, bool leader, bool quiet, int RS) {
//     _prepareOAgent_DFC(xbee,rx,G,leader,quiet);  
// }
...
//OGraph_DFC * OAgent_DFC::getGraph() {
//    return _G;
//}
//
//void OAgent_DFC::setLeader(bool leader) {
//    _leader = leader;
//}
//
//bool OAgent_DFC::isLeader() {
//    return _leader;
//}
//
//void OAgent_DFC::setQuiet(bool quiet) {
//    _quiet = quiet;
//}
//
//bool OAgent_DFC::isQuiet() {
//    return _quiet;
//}
...
/// Ratio-consensus
// Fair splitting
float OAgent_DFC::fairSplitRatioConsensus(long y, long z, uint8_t iterations, uint16_t period) {  //,uint8_t round
...
float OAgent_DFC::ratiomaxminConsensus(long y, long z, uint8_t iterations, uint16_t period) {  //,uint8_t round
    OLocalVertex * s = _G->getLocalVertex(); // store pointer to local vertex 
    float Dout = float(s->getOutDegree() + 1);    // store out degree, the +1 is to account for the self loops
...
// long OAgent_DFC::computeFairSplitFinalValue(float gamma) {
...
long OAgent_DFC::fairSplitRatioConsensus_RSL(long y, long z, uint8_t iterations, uint16_t period) {
...
long OAgent_DFC::leaderFairSplitRatioConsensus_RSL(long y, long z, uint8_t iterations, uint16_t period) {
...
long OAgent_DFC::nonleaderFairSplitRatioConsensus_RSL(long y, long z, uint8_t iterations, uint16_t period) {
...
long OAgent_DFC::leaderFairSplitRatioConsensus(long y, long z, uint8_t iterations, uint16_t period) {
    srand(analogRead(7));                    //moved this instruction here from fairSplitRatioConsensus() - Sammy
...
long OAgent_DFC::nonleaderFairSplitRatioConsensus(long y, long z) {
    srand(analogRead(7));
    unsigned long startTime = 0;
...
/// End ratio-consensus
/// Optimal dispatch
long OAgent_DFC::optimalDispatch(long x, uint8_t iterations, uint16_t period) {
...
bool OAgent_DFC::sync(uint8_t attempts) {
...
unsigned long OAgent_DFC::_broadcastResyncBeginPacket(unsigned long period){
...
void OAgent_DFC::_broadcastResyncResponsePacket(unsigned long t2, unsigned long received_id){
...
bool OAgent_DFC::_waitForResyncPacketResponse(unsigned long &rxTime, uint16_t timeout){

	return _waitForPacket(RESYNC_RESPONSE_HEADER, rxTime, true, timeout);      //assuming response is broadcasted from unsynced not
}

bool OAgent_DFC::_waitForResyncFinalPacket(unsigned long timeout){

	return _waitForPacket(RESYNC_HEADER_FINAL, false, timeout);
}

void OAgent_DFC::_broadcastResyncFinalPacket(long offset){
...
unsigned long OAgent_DFC::myMillis() { 
	return millis() - (unsigned long)_offset; 
}

void OAgent_DFC::_initializeFairSplitting(OLocalVertex * s, long y, long z) {
...
// Resilient version
void OAgent_DFC::_initializeFairSplitting_RSL(OLocalVertex * s, long y, long z) {
    _G->clearAllStates();                   // clear everything
    uint8_t Dout = s->getOutDegree() + 1;   // store out degree
...
void OAgent_DFC::_broadcastFairSplitPacket(OLocalVertex * s) {   
    uint16_t payload[7];           
    long mu    = s->getMuMin();
...
void OAgent_DFC::_broadcastFairSplitPacket_RSL(OLocalVertex * s) {   
    uint16_t payload[9];           
    long mu    = s->getMuMin();
    long sigma = s->getSigma();
    uint16_t id = s->getID();
...
long OAgent_DFC::_getMuFromPacket() {
    uint8_t ptr = 2;
    return _getUint32_tFromPacket(ptr);
}

long OAgent_DFC::_getSigmaFromPacket() {
    uint8_t ptr = 6;
    return _getUint32_tFromPacket(ptr);
}
...
void OAgent_DFC::_initializeOptimalDispatch(OLocalVertex * s, long x) {
...
void OAgent_DFC::_updateOptimalDispatchStates(OLocalVertex * s, uint8_t Dout) {
    // update 
    s->updateZ();
    s->updateSigma();
...
long OAgent_DFC::_findLambdaStar(OLocalVertex * s) {
    float min = 0;
    float max = 1000000;
    long lambdaMinus;
    long lambdaPlus;
...
void OAgent_DFC::_updateLambdaMinLambdaMax(OVertex * v, long &z, float &min, float &max, long &lambdaMinus, long &lambdaPlus) {
    uint8_t rsp;
...
uint8_t OAgent_DFC::_checkRatio(float &min, float &max, float ratio) {
...
void OAgent_DFC::_computeFinalOptimalDispatchRatios(OLocalVertex * s, float ratios[]) {
    long z = s->getZ();
    for(uint8_t i = 0; i < _G->getN(); i++) {
...
//float OAgent_DFC::_computeRatio(long num, long dem) {
//    return float(num)/float(dem);
//}

void OAgent_DFC::_findMinRatioMaxRatio(float ratios[], uint8_t num, float &min, float &max) {
    min = 0;
    max = 1000000;
...
void OAgent_DFC::_broadcastOptimalDispatchPacket(OLocalVertex * s) {
...
uint8_t OAgent_DFC::_getPayloadSize() {
 ...
 *
 */
uint8_t OAgent_DFC::_getNumberLambdaMinMax() {
    // initialize counter
    uint8_t l = 0;
    // check each vertex
    for(uint8_t i = 0; i < (_G->getN()); i++) {
...
uint8_t OAgent_DFC::_addDataToPayload(OVertex * v, uint8_t payload[], uint8_t ptr) {
...
uint8_t OAgent_DFC::_addUint32_tToPayload(uint32_t data, uint8_t payload[], uint8_t ptr) {
...
void OAgent_DFC::_processOptimalDispatchPacket(OLocalVertex * s, uint8_t i) {
...
void OAgent_DFC::_printFinalYMinYMax(OLocalVertex * s) {
    for(uint8_t i = 0; i < _G->getN(); i++)
        Serial << _DEC(i) << " yMin: " << _DEC(_G->getVertexByUniqueID(i)->getYMin()) << ", yMax: " << _DEC(_G->getVertexByUniqueID(i)->getYMax()) << endl;
}

void OAgent_DFC::_printStates(OLocalVertex * s, uint8_t k, bool printY) {
    uint8_t nodeNum = 1;
...
void OAgent_DFC::_printLambdas(OLocalVertex * s) {
    uint8_t nodeNum = 1;
    for(uint8_t i = 0; i < _G->getN(); i++) {
...
void OAgent_DFC::_printRatios(float ratios[], uint8_t num) {
    for(uint8_t i = 0; i < num; i++)
 ...
 */
bool OAgent_DFC::_validPacketAvailable() {
...
bool OAgent_DFC::_packetAvailable(uint16_t header, bool broadcast) {
    // read packet
    _xbee->readPacket();
    return _packetAvailableHelper(header,broadcast);
}

bool OAgent_DFC::_packetAvailable(uint16_t header,  unsigned long &rxTime, bool broadcast) {
...
uint16_t OAgent_DFC::_packetAvailable(bool broadcast) {
    _xbee->readPacket();
...
bool  OAgent_DFC::_packetAvailable2(unsigned long &rxTime, bool broadcast){
...
bool OAgent_DFC::_packetAvailableHelper(uint16_t header, bool broadcast) {
    //SerialUSB << "Here1\n";
    if(_validPacketAvailable()) {
        //SerialUSB << "HERE\n";
...
bool OAgent_DFC::_packetACKed(int timeout) {
 ...
 */
//uint16_t OAgent_DFC::_getHeaderFromPacket() {
//	return (uint16_t(_rx->getData(1)) << 8) + _rx->getData(0);
//}

bool OAgent_DFC::_waitForPacket(uint16_t header, unsigned long &rxTime, bool broadcast, int timeout) {
    unsigned long start;
...
bool OAgent_DFC::_waitForPacket(uint16_t header, bool broadcast, int timeout) { //Where it stays in an endless loop (if timeout =-1) until packet received
	unsigned long start;
    if(timeout != -1)
		start = millis();
	while(true) {
...
int  OAgent_DFC::_waitForPacket2(uint16_t header1, uint16_t header2, unsigned long &rxTime, bool broadcast){
...
uint16_t OAgent_DFC::_waitForValidPacket(bool broadcast, int timeout) {
    unsigned long start;
...
bool OAgent_DFC::_waitToStart(unsigned long startTime, bool useMyMillis, int timeout) {
    long temp;
 ...
 */
uint16_t OAgent_DFC::_genTxTime(uint16_t iterationPeriod, uint8_t ITF) {
	srand(analogRead(0));
 ...
 */
uint16_t OAgent_DFC::_genTxTime(uint16_t iterationPeriod, uint8_t ITF, int seed) {
	srand(seed);
	return (rand() % (iterationPeriod - 2*ITF)) + ITF;
}

bool OAgent_DFC::_timeToTransmit(uint16_t startTime, uint16_t txTime) {
...
void OAgent_DFC::_broadcastSchedulePacket(uint16_t header, unsigned long startTime, uint8_t numIterations, uint16_t period) {
    uint8_t payload[9];
...
void OAgent_DFC::_waitForSchedulePacket(uint16_t header, unsigned long &startTime, uint8_t &iterations, uint16_t &period, uint8_t id, int timeout) {
...
bool OAgent_DFC::_waitForSchedulePacket_RSL(uint16_t header, unsigned long &startTime, uint8_t &iterations, uint16_t &period, uint8_t id, int timeout) {
    if(_waitForPacket(header,true,timeout)) {  //stays in loop until desired packet received
        if(header == SCHEDULE_FAIR_SPLIT_HEADER || header == SCHEDULE_OPTIMAL_DISPATCH_HEADER)
        {
            startTime   = _getStartTimeFromPacket();
...
uint16_t OAgent_DFC::_waitForSchedulePacket(unsigned long &startTime, uint8_t &iterations, uint16_t &period, int timeout) {
...
void OAgent_DFC::_broadcastACKPacket(uint16_t header, uint8_t id)
{
    uint8_t payload[3];
    // put header in payload array
    payload[0] = header;
...
void OAgent_DFC::_WaitForACKPacket(uint16_t header, unsigned long t0, unsigned long startTime, uint8_t iterations, uint16_t period)
{   
...
//Resilient Version
bool OAgent_DFC::_WaitForACKPacket_RSL(uint16_t header, int timeout, unsigned long startTime, uint8_t iterations, uint16_t period )
{ 
    unsigned long start = millis();
...
uint32_t OAgent_DFC::_getAvailableAgentLsb(uint8_t i) {
    return _availableAgentLsb[i-1];
}

uint8_t OAgent_DFC::_getUint8_tFromPacket(uint8_t &byteNumber) {
    byteNumber++;
    return _rx->getData(byteNumber-1);
}

uint32_t OAgent_DFC::_getUint32_tFromPacket(uint8_t &lsbByteNumber) {
    lsbByteNumber += 4;
...
//long OAgent_DFC::_getLongFromPacket(uint8_t &lsbByteNumber) {
...
bool OAgent_DFC::_targetSync(unsigned long tTwo) {
...
bool OAgent_DFC::_nonTargetSync(unsigned long tTwo) {
...
unsigned long OAgent_DFC::_broadcastSyncBeginPacket(uint8_t i) {
    uint16_t payload[3];
...
void OAgent_DFC::_broadcastSyncFinalPacket(unsigned long tTwo, long d) {
	uint16_t payload[5];
...
bool OAgent_DFC::_unicastSyncResponsePacket(unsigned long tTwo) {
	uint16_t payload[3];
...
bool OAgent_DFC::_isTargetNode() {
    uint8_t ptr = 2;
    
	return _G->isLocalVertex(_getUint32_tFromPacket(ptr));
}

int OAgent_DFC:: getStatusData(int index)
{
	 OLocalVertex * s = _G->getLocalVertex();
...
void OAgent_DFC::_prepareOAgent_DFC(XBee * xbee, ZBRxResponse * rx, OGraph_DFC * G, bool leader, bool quiet) {
    _xbee = xbee;
    _G = G;
    _leader = leader;
    _quiet = quiet;
