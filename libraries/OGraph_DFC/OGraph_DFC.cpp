...
/// Public methods
// Constructors
OGraph_DFC::OGraph_DFC() {
    _n = 0;
    OLocalVertex s =  OLocalVertex();
    _self = &s;
}

OGraph_DFC::OGraph_DFC(OLocalVertex * s) {
    _n = 1;
    _self = s;
}

// Local vertex
//OLocalVertex * OGraph_DFC::getLocalVertex() {
//    return _self;
//}

bool OGraph_DFC::isLocalVertex(uint32_t aLsb) {
...
//bool OGraph_DFC::isLocalVertex(XBeeAddress64 a) {
//	return isLocalVertex(a.getLsb());
//}

// Graph properties
//uint8_t OGraph_DFC::getN() {
//    return _n;
//}

//uint8_t OGraph_DFC::getNumberNonNeighbors() {
//    return _n - _self->getInDegree() - 1;
//}

// Add and remove vertices from in-neighborhood
bool OGraph_DFC::addInNeighbor(XBeeAddress64 a) {
    return addInNeighbor(a.getLsb());
}

bool OGraph_DFC::addInNeighbor(uint32_t aLsb) {
    return addInNeighbor(aLsb,0,0);
}

bool OGraph_DFC::addInNeighbor(XBeeAddress64 a, long lambdaMin, long lambdaMax) {
    return addInNeighbor(a.getLsb(),lambdaMin,lambdaMax);
}

bool OGraph_DFC::addInNeighbor(uint32_t aLsb, long lambdaMin, long lambdaMax) {
...
bool OGraph_DFC::removeInNeighbor(int index){
...
void OGraph_DFC::AdjustVertexArray(int index, int step){
...
bool OGraph_DFC::isInNeighbor(XBeeAddress64 a) {
    return isInNeighbor(a.getLsb());
}

bool OGraph_DFC::isInNeighbor(uint32_t aLsb) {
    uint8_t i;
    return isInNeighbor(aLsb,i);
}

bool OGraph_DFC::isInNeighbor(XBeeAddress64 a, uint8_t &i) {
    return isInNeighbor(a.getLsb(),i);
}

bool OGraph_DFC::isInNeighbor(uint32_t aLsb, uint8_t &i) {
    if(_isRemoteVertex(aLsb,i))
        return _remoteVertices[i].isInNeighbor();
    return false;
}

bool OGraph_DFC::isInNeighbor(uint32_t aLsb, ORemoteVertex * &v) {
    uint8_t i;
...
bool OGraph_DFC::addNonNeighbor(XBeeAddress64 a) {
    return addNonNeighbor(a.getLsb());
}

bool OGraph_DFC::addNonNeighbor(uint32_t aLsb) {
    return addNonNeighbor(aLsb,0,0);
}

bool OGraph_DFC::addNonNeighbor(XBeeAddress64 a, long lambdaMin, long lambdaMax) {
    return addNonNeighbor(a.getLsb(),lambdaMin,lambdaMax);
}

bool OGraph_DFC::addNonNeighbor(uint32_t aLsb, long lambdaMin, long lambdaMax) {
    return _addRemoteVertex(aLsb,lambdaMin,lambdaMax);
}

// Determine if vertex is non-neighbor
bool OGraph_DFC::isNonNeighbor(XBeeAddress64 a) {
    return isNonNeighbor(a.getLsb());
}

bool OGraph_DFC::isNonNeighbor(uint32_t aLsb) {
    uint8_t i;
    return isNonNeighbor(aLsb,i);
}

bool OGraph_DFC::isNonNeighbor(XBeeAddress64 a, uint8_t &i) {
    return isNonNeighbor(a.getLsb(),i);
}

bool OGraph_DFC::isNonNeighbor(uint32_t aLsb, uint8_t &i) {
    if(_isRemoteVertex(aLsb,i))
        return !(_remoteVertices[i].isInNeighbor());
    return false;
}
...
void OGraph_DFC::clearAllYMinYMax() {
    _self->clearYMinYMax();
	for(uint8_t i = 0; i < NUM_REMOTE_VERTICES; i++)
		_remoteVertices[i].clearYMinYMax();
}

void OGraph_DFC::clearAllInStates() {
    _self->clearZIn();
...
void OGraph_DFC::clearAllBroadcastStates() {
	_self->clearSigma();
...
void OGraph_DFC::clearAllBroadcastLambda() {
    _self->clearBroadcastLambda();
    for(uint8_t i = 0; i < NUM_REMOTE_VERTICES; i++)
        _remoteVertices[i].clearBroadcastLambda();
}

void OGraph_DFC::clearAllStates() {
    _self->clearZInSigma();
    _self->setZ(0);
    _self->clearAllTau();
    _self->clearYMinYMax();
...
OVertex * OGraph_DFC::getVertexByAddressLsb(uint32_t aLsb) {
    uint8_t i;
    return getVertexByAddressLsb(aLsb,i);
}

// Methods to pointer to vertex
OVertex * OGraph_DFC::getVertexByAddressLsb(uint32_t aLsb, uint8_t &i) {
...
OVertex * OGraph_DFC::getVertexByAddress(XBeeAddress64 a) {
	return getVertexByAddressLsb(a.getLsb());
}

OVertex * OGraph_DFC::getVertexByUniqueID(uint8_t i) {
	if(i == 0) {
		return dynamic_cast<OVertex*> (_self);
...
ORemoteVertex * OGraph_DFC::getRemoteVertex(uint8_t i) {
...
uint8_t OGraph_DFC::_getRemoteVertexIndex(XBeeAddress64 a) {
	return _getRemoteVertexIndex(a.getLsb());
}

uint8_t OGraph_DFC::_getRemoteVertexIndex(uint32_t aLsb) {
	for(uint8_t i = 0; i < NUM_REMOTE_VERTICES; i++) {
...
bool OGraph_DFC::_isRemoteVertex(uint32_t aLsb) {
    uint8_t i = _getRemoteVertexIndex(aLsb);
    return _isRemoteVertex(aLsb,i);
}

bool OGraph_DFC::_isRemoteVertex(uint32_t aLsb, uint8_t &i) {
...
bool OGraph_DFC::_addRemoteVertex(uint32_t aLsb, long lambdaMin, long lambdaMax, bool inNeighbor) {
