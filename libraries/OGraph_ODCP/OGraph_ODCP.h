...
#ifndef OGraph_ODCP_h
#define OGraph_ODCP_h
...
class OVertex {
    public:
        ...
        inline void setY(long *y){  for(int i=0;i<4;i++) { _y[i] = y[i]; } }
        inline void setM(long *M){  for(int i=0;i<4;i++) { _M[i] = M[i]; } }
        inline void setm(long *m){  for(int i=0;i<4;i++) { _m[i] = m[i]; } }
        inline void setMalpha(long Malpha) { _Malpha = Malpha; }
        inline void setmalpha(long malpha) { _malpha = malpha; }
        ...
        inline long getY(uint8_t i) { return _y[i]; }
        ...
        inline void clearNuMin2(uint8_t j);
        // broadcast states
        inline void setMuMin(long muMin) { _muMin = muMin; }
        inline void setMu2(long * y, uint8_t Dout) {for(int i=0;i<4;i++) { _mu2[i] = y[i]/Dout; } }
        inline void setMu22(long * mu) { for(int i=0;i<4;i++) { _mu2[i] = mu[i]; } }
        inline void clearMu2(uint8_t k) { for(int i=0;i<k;i++) { _mu2[i] = 0; } }
        ...
        inline long getMu2(uint8_t i){ return _mu2[i];}
        inline long getM(uint8_t i){ return _M[i]; }
        inline long getm(uint8_t i){ return _m[i]; }
        inline long getMalpha() { return _Malpha;}
        inline long getmalpha() { return _malpha;}
        ...
        bool setNuMin2(uint8_t i, uint8_t j, long nuMin);
        bool setNuMax(uint8_t i, long nuMax);
        long getNuMin(uint8_t i);
        long getNuMin2(uint8_t i, uint8_t j);
    ...
    protected:
        ...
        long _yMin;
        long _yMax;
        long _y[4]; //since there are 4 nodes in the network
        long _M[4];
        long _m[4];
        long _Malpha;
        long _malpha; 
        ...
        long _nuMin2[NUM_IN_NEIGHBORS][4];       //each neighbor has 4 stacks of data
