...
bool OAgent_FF::standardFeasibleFlowAlgorithm(bool genBus, float alpha, uint8_t iterations) {
    ...
    for (uint8_t i:neighbors) {
        ...
        else{Serial<<"Initial flag is false for neighbor "<<i<<endl; delay(5);}
    }
    ...
    for(uint8_t k = 0; k < iterations; k++)
    {
        ...
        Serial << "Iteration " << k+1<<" "<<P<<" "<<bP <<endl;
        delay(5);
        ...
        uint8_t received_from[NUM_REMOTE_VERTICES];
        int i=0;
        start = millis();   // initialize timer
        while (millis()-start<=1000){
            ...
            else if(nei_to_send>nodeID) _unicastPacket_FF_P(nei_to_send,((n+(nei_to_send-1))->getActiveFlow()),self_flags[nei_to_send-1]);

            if(_waitForUnicastPacket(neighborID,nodeID,FF_HEADER,true,200))                                // Feasible Flow packet available for node from its neighbor
            {
                // Serial<<"neighbor is "<<neighborID<<endl; delay(5);
        ...
        for (uint8_t i:neighbors) {
            if (!received_from[i-1]){
                if (i>nodeID){
                    gp = alpha*bP; 
                    new_self_fp = ((n+(i-1))->getActiveFlow()) + gp;
                    if (new_self_fp>1) new_self_fp=1;
                    else if (new_self_fp<-1) new_self_fp=-1;
                    new_bP-=new_self_fp;
                }
                ...
                (n+(i-1))->setActiveFlow(new_self_fp);
            }
        }
        bP = new_bP;
