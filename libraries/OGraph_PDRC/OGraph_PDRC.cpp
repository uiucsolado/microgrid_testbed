...
OGraph_PDRC::OGraph_PDRC(OLocalVertex * s) {
    _n = 1;
    _self = s;
    //linkedlist for online neighbors
    LinkedList _l = LinkedList();
...
OGraph_PDRC::OGraph_PDRC(OLocalVertex * s, LinkedList * l) {
    _n = 1;
    _self = s;
    //linkedlist for online neighbors
    _list = l;
...
// Local vertex
//OLocalVertex * OGraph_PDRC::getLocalVertex() {
//    return _self;
//}

bool OGraph_PDRC::isLocalVertex(uint32_t aLsb) {
...
//bool OGraph_PDRC::isLocalVertex(XBeeAddress64 a) {
//	return isLocalVertex(a.getLsb());
//}

// Graph properties
//uint8_t OGraph_PDRC::getN() {
//    return _n;
//}

//uint8_t OGraph_PDRC::getNumberNonNeighbors() {
//    return _n - _self->getInDegree() - 1;
//}

// Add and remove vertices from in-neighborhood
bool OGraph_PDRC::addInNeighbor(XBeeAddress64 a, uint8_t neighborID) {
    return addInNeighbor(a.getLsb(),neighborID);
}

bool OGraph_PDRC::addInNeighbor(uint32_t aLsb, uint8_t neighborID) {
    return addInNeighbor(aLsb,neighborID,0,0);
}

bool OGraph_PDRC::addInNeighbor(XBeeAddress64 a, uint8_t neighborID, float r, float x) {
    return addInNeighbor(a.getLsb(),neighborID,r,x);
}

bool OGraph_PDRC::addInNeighbor(uint32_t aLsb, uint8_t neighborID, float r, float x) {
...
bool OGraph_PDRC::removeInNeighbor(uint8_t neighborID){
...
bool OGraph_PDRC::isInNeighbor(XBeeAddress64 a) {
    return isInNeighbor(a.getLsb());
}

bool OGraph_PDRC::isInNeighbor(uint32_t aLsb) {
    uint8_t i;
    return isInNeighbor(aLsb,i);
}

bool OGraph_PDRC::isInNeighbor(XBeeAddress64 a, uint8_t &i) {
    return isInNeighbor(a.getLsb(),i);
}

bool OGraph_PDRC::isInNeighbor(uint32_t aLsb, uint8_t &i) {
    if(_isRemoteVertex(aLsb,i))
        return _remoteVertices[i].isInNeighbor();
    return false;
}

bool OGraph_PDRC::isInNeighbor(uint32_t aLsb, ORemoteVertex * &v) {
    uint8_t i;
...
bool OGraph_PDRC::addNonNeighbor(XBeeAddress64 a, uint8_t neighborID) {
    return addNonNeighbor(a.getLsb(),neighborID);
}

bool OGraph_PDRC::addNonNeighbor(uint32_t aLsb, uint8_t neighborID) {
    return addNonNeighbor(aLsb,neighborID,0,0);
}

bool OGraph_PDRC::addNonNeighbor(XBeeAddress64 a, uint8_t neighborID, float r, float x) {
    return addNonNeighbor(a.getLsb(),neighborID,r,x);
}

bool OGraph_PDRC::addNonNeighbor(uint32_t aLsb, uint8_t neighborID, float r, float x) {
    return _addRemoteVertex(aLsb,neighborID,r,x);
}

// Determine if vertex is non-neighbor
bool OGraph_PDRC::isNonNeighbor(XBeeAddress64 a) {
    return isNonNeighbor(a.getLsb());
}

bool OGraph_PDRC::isNonNeighbor(uint32_t aLsb) {
    uint8_t i;
    return isNonNeighbor(aLsb,i);
}

bool OGraph_PDRC::isNonNeighbor(XBeeAddress64 a, uint8_t &i) {
    return isNonNeighbor(a.getLsb(),i);
}

bool OGraph_PDRC::isNonNeighbor(uint32_t aLsb, uint8_t &i) {
    if(_isRemoteVertex(aLsb,i))
        return !(_remoteVertices[i].isInNeighbor());
    return false;
}
...
void OGraph_PDRC::clearAllNuTau() {
	_self->clearAllTau();
...
void OGraph_PDRC::clearAllYMinYMax() {
    _self->clearYMinYMax();
	for(uint8_t i = 0; i < NUM_REMOTE_VERTICES; i++)
		_remoteVertices[i].clearYMinYMax();
}

void OGraph_PDRC::clearAllInStates() {
    _self->clearZIn();
...
void OGraph_PDRC::clearAllBroadcastStates() {
	_self->clearSigma();
...
void OGraph_PDRC::clearAllStates() {
    _self->clearZInSigma();
    _self->setZ(0);
    _self->clearAllTau();
    _self->clearYMinYMax();
...
OVertex * OGraph_PDRC::getVertexByAddressLsb(uint32_t aLsb) {
    uint8_t i;
    return getVertexByAddressLsb(aLsb,i);
}

// Methods to pointer to vertex
OVertex * OGraph_PDRC::getVertexByAddressLsb(uint32_t aLsb, uint8_t &i) {
...
OVertex * OGraph_PDRC::getVertexByAddress(XBeeAddress64 a) {
	return getVertexByAddressLsb(a.getLsb());
}

OVertex * OGraph_PDRC::getVertexByUniqueID(uint8_t i) {
	if(i == 0) {
		return dynamic_cast<OVertex*> (_self);
...
ORemoteVertex * OGraph_PDRC::getRemoteVertex(uint8_t neighborID) {
...
void OGraph_PDRC::configureLinkedList() {
    _list->updateLinkedList(_self->getStatusP());
    _list->displayLinkedList();
    Serial<<"Out degree is: "<<_self->getOutDegree()<<endl;
}

uint8_t OGraph_PDRC::_getRemoteVertexIndex(XBeeAddress64 a) {
    return _getRemoteVertexIndex(a.getLsb());
}

uint8_t OGraph_PDRC::_getRemoteVertexIndex(uint32_t aLsb) {
	for(uint8_t i = 0; i < NUM_REMOTE_VERTICES; i++) {
...
bool OGraph_PDRC::_isRemoteVertex(uint32_t aLsb) {
    uint8_t i = _getRemoteVertexIndex(aLsb);
    return _isRemoteVertex(aLsb,i);
}

bool OGraph_PDRC::_isRemoteVertex(uint32_t aLsb, uint8_t &i) {
...
bool OGraph_PDRC::_addRemoteVertex(uint32_t aLsb, uint8_t neighborID, float r, float x, bool inNeighbor) {
