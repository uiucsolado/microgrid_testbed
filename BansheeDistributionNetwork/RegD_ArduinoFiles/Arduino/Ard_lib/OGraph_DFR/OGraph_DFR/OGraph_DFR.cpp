...
/// Public methods
// Constructors
OGraph_DFR::OGraph_DFR() {
    _n = 0;
    OLocalVertex s =  OLocalVertex();
    _self = &s;
}

OGraph_DFR::OGraph_DFR(OLocalVertex * s) {
    _n = 1;
    _self = s;
}

// Local vertex
//OLocalVertex * OGraph_DFR::getLocalVertex() {
//    return _self;
//}

bool OGraph_DFR::isLocalVertex(uint32_t aLsb) {
...
//bool OGraph_DFR::isLocalVertex(XBeeAddress64 a) {
//	return isLocalVertex(a.getLsb());
//}

// Graph properties
//uint8_t OGraph_DFR::getN() {
//    return _n;
//}

//uint8_t OGraph_DFR::getNumberNonNeighbors() {
//    return _n - _self->getInDegree() - 1;
//}

// Add and remove vertices from in-neighborhood
bool OGraph_DFR::addInNeighbor(XBeeAddress64 a) {
    return addInNeighbor(a.getLsb());
}

bool OGraph_DFR::addInNeighbor(uint32_t aLsb) {
    return addInNeighbor(aLsb,0,0);
}

bool OGraph_DFR::addInNeighbor(XBeeAddress64 a, long lambdaMin, long lambdaMax) {
    return addInNeighbor(a.getLsb(),lambdaMin,lambdaMax);
}

bool OGraph_DFR::addInNeighbor(uint32_t aLsb, long lambdaMin, long lambdaMax) {
...
bool OGraph_DFR::removeInNeighbor(int index){
...
void OGraph_DFR::AdjustVertexArray(int index, int step){
...
bool OGraph_DFR::isInNeighbor(XBeeAddress64 a) {
    return isInNeighbor(a.getLsb());
}

bool OGraph_DFR::isInNeighbor(uint32_t aLsb) {
    uint8_t i;
    return isInNeighbor(aLsb,i);
}

bool OGraph_DFR::isInNeighbor(XBeeAddress64 a, uint8_t &i) {
    return isInNeighbor(a.getLsb(),i);
}

bool OGraph_DFR::isInNeighbor(uint32_t aLsb, uint8_t &i) {
    if(_isRemoteVertex(aLsb,i))
        return _remoteVertices[i].isInNeighbor();
    return false;
}

bool OGraph_DFR::isInNeighbor(uint32_t aLsb, ORemoteVertex * &v) {
    uint8_t i;
...
bool OGraph_DFR::addNonNeighbor(XBeeAddress64 a) {
    return addNonNeighbor(a.getLsb());
}

bool OGraph_DFR::addNonNeighbor(uint32_t aLsb) {
    return addNonNeighbor(aLsb,0,0);
}

bool OGraph_DFR::addNonNeighbor(XBeeAddress64 a, long lambdaMin, long lambdaMax) {
    return addNonNeighbor(a.getLsb(),lambdaMin,lambdaMax);
}

bool OGraph_DFR::addNonNeighbor(uint32_t aLsb, long lambdaMin, long lambdaMax) {
    return _addRemoteVertex(aLsb,lambdaMin,lambdaMax);
}

// Determine if vertex is non-neighbor
bool OGraph_DFR::isNonNeighbor(XBeeAddress64 a) {
    return isNonNeighbor(a.getLsb());
}

bool OGraph_DFR::isNonNeighbor(uint32_t aLsb) {
    uint8_t i;
    return isNonNeighbor(aLsb,i);
}

bool OGraph_DFR::isNonNeighbor(XBeeAddress64 a, uint8_t &i) {
    return isNonNeighbor(a.getLsb(),i);
}

bool OGraph_DFR::isNonNeighbor(uint32_t aLsb, uint8_t &i) {
    if(_isRemoteVertex(aLsb,i))
        return !(_remoteVertices[i].isInNeighbor());
    return false;
}
...
void OGraph_DFR::clearAllYMinYMax() {
    _self->clearYMinYMax();
	for(uint8_t i = 0; i < NUM_REMOTE_VERTICES; i++)
		_remoteVertices[i].clearYMinYMax();
}

void OGraph_DFR::clearAllInStates() {
    _self->clearZIn();
...
void OGraph_DFR::clearAllBroadcastStates() {
	_self->clearSigma();
...
void OGraph_DFR::clearAllBroadcastLambda() {
    _self->clearBroadcastLambda();
    for(uint8_t i = 0; i < NUM_REMOTE_VERTICES; i++)
        _remoteVertices[i].clearBroadcastLambda();
}

void OGraph_DFR::clearAllStates() {
    _self->clearZInSigma();
    _self->setZ(0);
    _self->clearAllTau();
    _self->clearYMinYMax();
...
OVertex * OGraph_DFR::getVertexByAddressLsb(uint32_t aLsb) {
    uint8_t i;
    return getVertexByAddressLsb(aLsb,i);
}

// Methods to pointer to vertex
OVertex * OGraph_DFR::getVertexByAddressLsb(uint32_t aLsb, uint8_t &i) {
...
OVertex * OGraph_DFR::getVertexByAddress(XBeeAddress64 a) {
	return getVertexByAddressLsb(a.getLsb());
}

OVertex * OGraph_DFR::getVertexByUniqueID(uint8_t i) {
	if(i == 0) {
		return dynamic_cast<OVertex*> (_self);
...
ORemoteVertex * OGraph_DFR::getRemoteVertex(uint8_t i) {
...
uint8_t OGraph_DFR::_getRemoteVertexIndex(XBeeAddress64 a) {
	return _getRemoteVertexIndex(a.getLsb());
}

uint8_t OGraph_DFR::_getRemoteVertexIndex(uint32_t aLsb) {
	for(uint8_t i = 0; i < NUM_REMOTE_VERTICES; i++) {
...
bool OGraph_DFR::_isRemoteVertex(uint32_t aLsb) {
    uint8_t i = _getRemoteVertexIndex(aLsb);
    return _isRemoteVertex(aLsb,i);
}

bool OGraph_DFR::_isRemoteVertex(uint32_t aLsb, uint8_t &i) {
...
bool OGraph_DFR::_addRemoteVertex(uint32_t aLsb, long lambdaMin, long lambdaMax, bool inNeighbor) {
