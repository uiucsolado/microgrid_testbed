...
long base = 1000;  // not using floating points so need a base number
...
OLocalVertex s = OLocalVertex(0x4151C48B,0,0.0679*D_base,-1.5*base,0.5*base,5,D_base,4);
//OLocalVertex s = OLocalVertex(0x4151C48B,0,0,-1.5*base,0.5*base,5,D_base);
OGraph g = OGraph(&s);
  ...
  if(de == false) {
   Serial.println("Still trying to sync");
    ...
    if(a.isSynced()) {
      ...
      //a.nonleaderFairSplitRatioConsensus(D_base,5);
      ...
      /*    
      n = state*base_value;//regd*base_value; 
