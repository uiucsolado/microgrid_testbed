...
OLocalVertex s = OLocalVertex(0x4151C688,0,0.95*D_base,-1.5*base,0.5*base,5,base,5);
//OLocalVertex s = OLocalVertex(0x4151C688,0,0,-1.5*base,0.5*base,5,base);
OGraph g = OGraph(&s);
  ...
  if(de == false) {
   Serial.println("Still trying to sync");
    ...
    if(a.isSynced()) {
      ...
      //a.nonleaderFairSplitRatioConsensus(D_base,3);
...
void sendConsensusResults()
{
  ...
  ////////////////////////////////////////////////////////////////
  int node3_ip = 63; //part of ip address for node 1 on the HIL side 
  Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,0,2,1,node3_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos - arduino, int nodeip)
  Mb.MbmRun();
...
{
  int node3_ip = 63; //part of ip address for node 1 on the HIL side 
  Mb.Req(MB_FC_READ_INPUT_REGISTER,0,1,0,node3_ip); //(MB_FC FC, word Ref, word Count, word Pos, int nodeip)
  Mb.MbmRun();
}
