...
void loop() {
  ...
  else 
  {
    if(a.isSynced())
    {
      if (a.isLeader())
      {
        Serial.println("Begin maxmin consensus? (y/n)"); //let computer know you want to begin ratio consensus
        while (Serial.available() == 0) 
        { 
          //simply makes the arduino wait until commputer sends signal        
        }
        if(Serial.available()) 
        {
          ...
          if (o == 'y')
          {
            MMC = a.maxminConsensusAlgorithm(!true,20,6,5,150);
            Serial.println("MMC result");
            Serial.println(MMC,4);
          }
        }
      }
      if (!(a.isLeader()))
