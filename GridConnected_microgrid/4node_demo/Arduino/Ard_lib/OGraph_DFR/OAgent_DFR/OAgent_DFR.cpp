...
//// Public methods
/// Constructors
OAgent_DFR::OAgent_DFR() {

    XBee temp1 = XBee();
    ZBRxResponse temp2 = ZBRxResponse();
    OGraph_DFR temp3 = OGraph_DFR();
    _prepareOAgent_DFR(&temp1,&temp2,&temp3);
    //setRS(0);
}

OAgent_DFR::OAgent_DFR(XBee * xbee, OGraph_DFR * G, bool leader, bool quiet) {
     ZBRxResponse temp = ZBRxResponse();
    _prepareOAgent_DFR(xbee,&temp,G,leader,quiet);
    //setRS(0);
}

OAgent_DFR::OAgent_DFR(XBee * xbee, ZBRxResponse * rx, OGraph_DFR * G, bool leader, bool quiet) {
    _prepareOAgent_DFR(xbee,rx,G,leader,quiet);
    //setRS(0);
}

// OAgent_DFR::OAgent_DFR(XBee * xbee, ZBRxResponse * rx, OGraph_DFR * G, bool leader, bool quiet, int RS) {
//     _prepareOAgent_DFR(xbee,rx,G,leader,quiet);  
// }
...
//OGraph_DFR * OAgent_DFR::getGraph() {
//    return _G;
//}
//
//void OAgent_DFR::setLeader(bool leader) {
//    _leader = leader;
//}
//
//bool OAgent_DFR::isLeader() {
//    return _leader;
//}
//
//void OAgent_DFR::setQuiet(bool quiet) {
//    _quiet = quiet;
//}
//
//bool OAgent_DFR::isQuiet() {
//    return _quiet;
//}
...
/// Ratio-consensus
// Fair splitting
float OAgent_DFR::fairSplitRatioConsensus(long y, long z, uint8_t iterations, uint16_t period) {  //,uint8_t round
...
float OAgent_DFR::ratiomaxminConsensus(long y, long z, uint8_t iterations, uint16_t period) {  //,uint8_t round
    OLocalVertex * s = _G->getLocalVertex(); // store pointer to local vertex 
    float Dout = float(s->getOutDegree() + 1);    // store out degree, the +1 is to account for the self loops
...
// long OAgent_DFR::computeFairSplitFinalValue(float gamma) {
...
long OAgent_DFR::fairSplitRatioConsensus_RSL(long y, long z, uint8_t iterations, uint16_t period) {
    ...
    Serial<<"Sup bro3! "<<getbufferdata(0)<<"\n";
    if (gamma!=-1)
        return gamma;
    else
        return 0;
...
long OAgent_DFR::leaderFairSplitRatioConsensus_RSL(long y, long z, uint8_t iterations, uint16_t period) {
    ...
    else
    {
        if(_waitToStart(startTime,true,1800))
        {
            ...
            Serial<<"Sup bro2! "<<getbufferdata(0)<<"\n";
        }
    }        
    return gamma;
}

long OAgent_DFR::nonleaderFairSplitRatioConsensus_RSL(long y, long z, uint8_t iterations, uint16_t period) {
...
long OAgent_DFR::leaderFairSplitRatioConsensus(long y, long z, uint8_t iterations, uint16_t period) {
    srand(analogRead(7));                    //moved this instruction here from fairSplitRatioConsensus() - Sammy
...
long OAgent_DFR::nonleaderFairSplitRatioConsensus(long y, long z) {
    srand(analogRead(7));
    unsigned long startTime = 0;
...
/// End ratio-consensus
/// Optimal dispatch
long OAgent_DFR::optimalDispatch(long x, uint8_t iterations, uint16_t period) {
...
bool OAgent_DFR::sync(uint8_t attempts) {
...
unsigned long OAgent_DFR::_broadcastResyncBeginPacket(unsigned long period){
...
void OAgent_DFR::_broadcastResyncResponsePacket(unsigned long t2, unsigned long received_id){
...
bool OAgent_DFR::_waitForResyncPacketResponse(unsigned long &rxTime, uint16_t timeout){

	return _waitForPacket(RESYNC_RESPONSE_HEADER, rxTime, true, timeout);      //assuming response is broadcasted from unsynced not
}

bool OAgent_DFR::_waitForResyncFinalPacket(unsigned long timeout){

	return _waitForPacket(RESYNC_HEADER_FINAL, false, timeout);
}

void OAgent_DFR::_broadcastResyncFinalPacket(long offset){
...
unsigned long OAgent_DFR::myMillis() { 
	return millis() - (unsigned long)_offset; 
}

void OAgent_DFR::_initializeFairSplitting(OLocalVertex * s, long y, long z) {
...
// Resilient version
void OAgent_DFR::_initializeFairSplitting_RSL(OLocalVertex * s, long y, long z) {
    _G->clearAllStates();                   // clear everything
    uint8_t Dout = s->getOutDegree() + 1;   // store out degree
...
void OAgent_DFR::_broadcastFairSplitPacket(OLocalVertex * s) {   
    uint16_t payload[6];           
    long mu    = s->getMuMin();
...
void OAgent_DFR::_broadcastFairSplitPacket_RSL(OLocalVertex * s) {   
    uint16_t payload[6];           
    long mu    = s->getMuMin();
    long sigma = s->getSigma();
    uint16_t id = s->getID();
...
long OAgent_DFR::_getMuFromPacket() {
    uint8_t ptr = 2;
    return _getUint32_tFromPacket(ptr);
}

long OAgent_DFR::_getSigmaFromPacket() {
    uint8_t ptr = 6;
    return _getUint32_tFromPacket(ptr);
}
...
void OAgent_DFR::_initializeOptimalDispatch(OLocalVertex * s, long x) {
...
void OAgent_DFR::_updateOptimalDispatchStates(OLocalVertex * s, uint8_t Dout) {
    // update 
    s->updateZ();
    s->updateSigma();
...
long OAgent_DFR::_findLambdaStar(OLocalVertex * s) {
    float min = 0;
    float max = 1000000;
    long lambdaMinus;
    long lambdaPlus;
...
void OAgent_DFR::_updateLambdaMinLambdaMax(OVertex * v, long &z, float &min, float &max, long &lambdaMinus, long &lambdaPlus) {
    uint8_t rsp;
...
uint8_t OAgent_DFR::_checkRatio(float &min, float &max, float ratio) {
...
void OAgent_DFR::_computeFinalOptimalDispatchRatios(OLocalVertex * s, float ratios[]) {
    long z = s->getZ();
    for(uint8_t i = 0; i < _G->getN(); i++) {
...
//float OAgent_DFR::_computeRatio(long num, long dem) {
//    return float(num)/float(dem);
//}

void OAgent_DFR::_findMinRatioMaxRatio(float ratios[], uint8_t num, float &min, float &max) {
    min = 0;
    max = 1000000;
...
void OAgent_DFR::_broadcastOptimalDispatchPacket(OLocalVertex * s) {
...
uint8_t OAgent_DFR::_getPayloadSize() {
 ...
 *
 */
uint8_t OAgent_DFR::_getNumberLambdaMinMax() {
    // initialize counter
    uint8_t l = 0;
    // check each vertex
    for(uint8_t i = 0; i < (_G->getN()); i++) {
...
uint8_t OAgent_DFR::_addDataToPayload(OVertex * v, uint8_t payload[], uint8_t ptr) {
...
uint8_t OAgent_DFR::_addUint32_tToPayload(uint32_t data, uint8_t payload[], uint8_t ptr) {
...
void OAgent_DFR::_processOptimalDispatchPacket(OLocalVertex * s, uint8_t i) {
...
void OAgent_DFR::_printFinalYMinYMax(OLocalVertex * s) {
    for(uint8_t i = 0; i < _G->getN(); i++)
        Serial << _DEC(i) << " yMin: " << _DEC(_G->getVertexByUniqueID(i)->getYMin()) << ", yMax: " << _DEC(_G->getVertexByUniqueID(i)->getYMax()) << endl;
}

void OAgent_DFR::_printStates(OLocalVertex * s, uint8_t k, bool printY) {
    uint8_t nodeNum = 1;
...
void OAgent_DFR::_printLambdas(OLocalVertex * s) {
    uint8_t nodeNum = 1;
    for(uint8_t i = 0; i < _G->getN(); i++) {
...
void OAgent_DFR::_printRatios(float ratios[], uint8_t num) {
    for(uint8_t i = 0; i < num; i++)
 ...
 */
bool OAgent_DFR::_validPacketAvailable() {
...
bool OAgent_DFR::_packetAvailable(uint16_t header, bool broadcast) {
    // read packet
    _xbee->readPacket();
    return _packetAvailableHelper(header,broadcast);
}

bool OAgent_DFR::_packetAvailable(uint16_t header,  unsigned long &rxTime, bool broadcast) {
...
uint16_t OAgent_DFR::_packetAvailable(bool broadcast) {
    _xbee->readPacket();
...
bool  OAgent_DFR::_packetAvailable2(unsigned long &rxTime, bool broadcast){
...
bool OAgent_DFR::_packetAvailableHelper(uint16_t header, bool broadcast) {
    //SerialUSB << "Here1\n";
    if(_validPacketAvailable()) {
        //SerialUSB << "HERE\n";
...
bool OAgent_DFR::_packetACKed(int timeout) {
 ...
 */
//uint16_t OAgent_DFR::_getHeaderFromPacket() {
//	return (uint16_t(_rx->getData(1)) << 8) + _rx->getData(0);
//}

bool OAgent_DFR::_waitForPacket(uint16_t header, unsigned long &rxTime, bool broadcast, int timeout) {
    unsigned long start;
...
bool OAgent_DFR::_waitForPacket(uint16_t header, bool broadcast, int timeout) { //Where it stays in an endless loop (if timeout =-1) until packet received
	unsigned long start;
    if(timeout != -1)
		start = millis();
	while(true) {
...
int  OAgent_DFR::_waitForPacket2(uint16_t header1, uint16_t header2, unsigned long &rxTime, bool broadcast){
...
uint16_t OAgent_DFR::_waitForValidPacket(bool broadcast, int timeout) {
    unsigned long start;
...
bool OAgent_DFR::_waitToStart(unsigned long startTime, bool useMyMillis, int timeout) {
    long temp;
 ...
 */
uint16_t OAgent_DFR::_genTxTime(uint16_t iterationPeriod, uint8_t ITF) {
	srand(analogRead(0));
 ...
 */
uint16_t OAgent_DFR::_genTxTime(uint16_t iterationPeriod, uint8_t ITF, int seed) {
	srand(seed);
	return (rand() % (iterationPeriod - 2*ITF)) + ITF;
}

bool OAgent_DFR::_timeToTransmit(uint16_t startTime, uint16_t txTime) {
...
void OAgent_DFR::_broadcastSchedulePacket(uint16_t header, unsigned long startTime, uint8_t numIterations, uint16_t period) {
    uint8_t payload[9];
...
void OAgent_DFR::_waitForSchedulePacket(uint16_t header, unsigned long &startTime, uint8_t &iterations, uint16_t &period, uint8_t id, int timeout) {
...
bool OAgent_DFR::_waitForSchedulePacket_RSL(uint16_t header, unsigned long &startTime, uint8_t &iterations, uint16_t &period, uint8_t id, int timeout) {
    if(_waitForPacket(header,true,timeout)) {  //stays in loop until desired packet received
        if(header == SCHEDULE_FAIR_SPLIT_HEADER || header == SCHEDULE_OPTIMAL_DISPATCH_HEADER)
        {
            startTime   = _getStartTimeFromPacket();
...
uint16_t OAgent_DFR::_waitForSchedulePacket(unsigned long &startTime, uint8_t &iterations, uint16_t &period, int timeout) {
...
void OAgent_DFR::_broadcastACKPacket(uint16_t header, uint8_t id)
{
    uint8_t payload[3];
    // put header in payload array
    payload[0] = header;
...
void OAgent_DFR::_WaitForACKPacket(uint16_t header, unsigned long t0, unsigned long startTime, uint8_t iterations, uint16_t period)
{   
...
//Resilient Version
bool OAgent_DFR::_WaitForACKPacket_RSL(uint16_t header, int timeout, unsigned long startTime, uint8_t iterations, uint16_t period )
{ 
    unsigned long start = millis();
...
uint32_t OAgent_DFR::_getAvailableAgentLsb(uint8_t i) {
    return _availableAgentLsb[i-1];
}

uint8_t OAgent_DFR::_getUint8_tFromPacket(uint8_t &byteNumber) {
    byteNumber++;
    return _rx->getData(byteNumber-1);
}

uint32_t OAgent_DFR::_getUint32_tFromPacket(uint8_t &lsbByteNumber) {
    lsbByteNumber += 4;
...
//long OAgent_DFR::_getLongFromPacket(uint8_t &lsbByteNumber) {
...
bool OAgent_DFR::_targetSync(unsigned long tTwo) {
...
bool OAgent_DFR::_nonTargetSync(unsigned long tTwo) {
...
unsigned long OAgent_DFR::_broadcastSyncBeginPacket(uint8_t i) {
    uint16_t payload[3];
...
void OAgent_DFR::_broadcastSyncFinalPacket(unsigned long tTwo, long d) {
	uint16_t payload[5];
...
bool OAgent_DFR::_unicastSyncResponsePacket(unsigned long tTwo) {
	uint16_t payload[3];
...
bool OAgent_DFR::_isTargetNode() {
    uint8_t ptr = 2;
    
	return _G->isLocalVertex(_getUint32_tFromPacket(ptr));
}

int OAgent_DFR:: getStatusData(int index)
{
	 OLocalVertex * s = _G->getLocalVertex();
...
void OAgent_DFR::_prepareOAgent_DFR(XBee * xbee, ZBRxResponse * rx, OGraph_DFR * G, bool leader, bool quiet) {
    _xbee = xbee;
    _G = G;
    _leader = leader;
    _quiet = quiet;
