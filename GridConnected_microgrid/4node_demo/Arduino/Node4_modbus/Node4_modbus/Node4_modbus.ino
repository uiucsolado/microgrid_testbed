...
long capacity_max = 10000;  //DER capacity_max limit
long capacity_min = -5000;  //DER capacity_min limit
...
OLocalVertex s = OLocalVertex(0x4151C48B,0,iP_nom*base,-2.1667*base,0.1667*base,2,base,4);  //#NODE
...
void sendConsensusResults()
{
  ...
  Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,0,3,0,node4_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos - arduino, int nodeip) //#NODE
  Mb.MbmRun();
...
{
  int node4_ip = 64; //part of ip address for node 4 on the HIL side
  Mb.Req(MB_FC_READ_INPUT_REGISTER,0,3,0,node4_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos -arduino, int nodeip) //#NODE
  Mb.MbmRun();
}
