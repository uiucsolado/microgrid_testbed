...
OLocalVertex s = OLocalVertex(0x4174F1AA,0,iP_nom*base,-2.1667*base,0.1667*base,2,base,1);  //#NODE
OGraph_DFR g = OGraph_DFR(&s);
OAgent_DFR a = OAgent_DFR(&xbee,&rx,&g,true,true); // argument rx?
...
void sendConsensusResults()
{
  ...
  Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,0,3,0,node1_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos - arduino, int nodeip) //#NODE
  Mb.MbmRun();
...
void receiveTyphoonData()
{
  ...
  Mb.Req(MB_FC_READ_INPUT_REGISTER,0,3,0,node1_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos -arduino, int nodeip) //#NODE
  Mb.MbmRun();
}
