...
OLocalVertex s = OLocalVertex(0x4174F186,0,capacity*base,-1.5*base,0.5*base,2,base, node_id);
OGraph g = OGraph(&s);
...
void sendConsensusResults()
{
  ...
  ref = 10;                                //starting address of node 2 registers is 10
  ...
  int node2_ip = 42; //part of ip address for node 1 on the HIL side
  Mb.Build(fc,Ref_high,Ref_low,Count_high,Count_low,Pos_high,Pos_low,node2_ip);
  Mb.MbmRun();
