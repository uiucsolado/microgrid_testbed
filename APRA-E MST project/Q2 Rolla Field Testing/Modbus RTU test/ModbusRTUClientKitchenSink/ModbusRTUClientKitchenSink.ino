...
void readHoldingRegisterValues() {
  ...
  if (!ModbusRTUClient.requestFrom(4, HOLDING_REGISTERS, 0x9C44, 2)) {
    Serial.print("failed! ");
    Serial.println(ModbusRTUClient.lastError());
  } else {
    Serial.println("success");
...
void ReadPowerMeter() {
  Serial.print("Reading Power Meter ... ");
  delay(100);
  // read 10 Input Register values from (slave) id 42, address 0x00
  if (!ModbusRTUClient.requestFrom(4, HOLDING_REGISTERS, 0x9C93, 1)) {
    Serial.print("failed! ");
    Serial.println(ModbusRTUClient.lastError());
  } else {
    Serial.println("success");
...
void EnablePowerReduction() {
  // set the Holding Register values to counter

  Serial.print("Enable Power Reduction ... ");

  // write 10 coil values to (slave) id 42, address 0x00
  ModbusRTUClient.beginTransmission(4, HOLDING_REGISTERS, 0x9D28, 1);
  ModbusRTUClient.write(50);
  if (!ModbusRTUClient.endTransmission()) {
    Serial.print("failed! ");
    Serial.println(ModbusRTUClient.lastError());
  } else {
  ...
  ModbusRTUClient.beginTransmission(4, HOLDING_REGISTERS, 0x9D2C, 1);
