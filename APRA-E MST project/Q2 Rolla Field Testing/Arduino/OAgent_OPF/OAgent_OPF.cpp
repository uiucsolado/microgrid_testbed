...
float OAgent_OPF::dual_function(float alpha_p, float beta_p, float max_p, float min_p, float lambda){

    float lambdalo = (min_p-alpha_p)/beta_p; float lambdahi = (max_p-alpha_p)/beta_p;
    if (lambda<lambdalo) return min_p;
    if (lambda>lambdahi) return max_p;
    return alpha_p + lambda*beta_p;
...
float OAgent_OPF::economicDispatch(float alpha_p, float beta_p, float max_p, float min_p, float u, uint16_t iterations, uint16_t period) {
    ...
    for (int i=0;i<deg;i++){
        ...
        // Serial <<"Neighbor "<<neighbors[i]<<" ->index "<<i<<endl;
        // delay(5);
    } 
    ...
    uint8_t arraySize = 6;
    float * lambdalo = new float[arraySize];float * lambdahi = new float[arraySize];
    uint8_t receivedlo = 1<<(nodeID-1);uint8_t receivedhi = 1<<(nodeID-1);
    // Serial<<"Received arrays "<<receivedlo<<" "<<receivedhi<<endl;

    float * Ylo = new float[arraySize];float * Yhi = new float[arraySize];
    float* ratioslo = new float[arraySize];float* ratioshi = new float[arraySize];
    for(uint8_t i = 0; i < arraySize; i++){ 
        Ylo[i] = 0;Yhi[i] = 0;
        lambdalo[i] = 0;lambdahi[i] = 0;
        ratioslo[i] = 0;ratioshi[i] = 0;
    }    
    lambdalo[nodeID-1] = (min_p-alpha_p)/beta_p; lambdahi[nodeID-1] = (max_p-alpha_p)/beta_p;
    Ylo[nodeID-1] = dual_function(alpha_p,beta_p,max_p,min_p,lambdalo[nodeID-1]);
    Yhi[nodeID-1] = dual_function(alpha_p,beta_p,max_p,min_p,lambdahi[nodeID-1]);
    // Serial<<"Ylo and Yhi "<<Ylo[nodeID-1]<<" "<<Yhi[nodeID-1]<<endl;

    float SumYlo[1+deg][arraySize];float SumYhi[1+deg][arraySize];
    for (int i=0;i<1+deg;i++){
        for (int j=0;j<arraySize;j++){SumYlo[i][j]=0;SumYhi[i][j]=0;}
    }
	for(uint8_t i = 0; i < arraySize; i++){
        SumYlo[0][i]+= Ylo[i]/Dout; 
        SumYhi[0][i]+= Yhi[i]/Dout;
    }
    
	float Zlo = u;float Zhi = u;
	float * SumZlo = new float[1+deg];float * SumZhi = new float[1+deg];
	for(uint8_t i = 0; i < 1+deg; i++){ SumZlo[i] = 0;SumZhi[i] = 0;}
	SumZlo[0]  += Zlo/Dout; SumZhi[0]  += Zhi/Dout;
    
    bool comm_in_neighbor_lo[deg];bool comm_in_neighbor_hi[deg];
	for(uint8_t i = 0; i < deg; i++){ comm_in_neighbor_lo[i]=false;comm_in_neighbor_hi[i]=false;}
	bool comm_with_neighbor_hi = false; bool comm_with_neighbor_lo = false;
 ...
 //    while (!scheduled)
 //    { 
 //    	if(isLeader()) scheduled = _ScheduleLeaderOPF(startTime,iters);
 //    	else  scheduled = _ScheduleNonLeaderOPF(startTime,iters);
	// }
    ...
    for(uint8_t k = 0; k < iterations; k++)
    {
        ...
        txDone = false;
        float * newYlo = new float[arraySize];
        float * newYhi = new float[arraySize];
        float newZlo = 0; float newZhi=0;
        for(uint8_t i = 0; i < arraySize; i++){
            newYlo[i] = 0;newYhi[i] = 0;
            if (comm_with_neighbor_lo) SumYlo[0][i]+= Ylo[i]/Dout;
            if (comm_with_neighbor_hi) SumYhi[0][i]+= Yhi[i]/Dout;
        }
        if (comm_with_neighbor_lo) SumZlo[0]  += Zlo/Dout;
        if (comm_with_neighbor_hi) SumZhi[0]  += Zhi/Dout;

        start = millis();   // initialize timer
        uint8_t next_receivedhi = receivedhi; uint8_t next_receivedlo = receivedlo;
        while( uint16_t(millis()-start) < period )
        {
            receivedPacket = _waitForNeighborPacket(neighborID,ED_HEADER,true,100);
            // Serial << "Data received "<<receivedPacket<<endl;
            if ( receivedPacket && !(comm_in_neighbor_lo[nei2index[neighborID-1]] || comm_in_neighbor_hi[nei2index[neighborID-1]]) )                                                                         //get values for fp, fq, and lambda that are received from this neighbor
            {

            	if (millis()%3==1) {_broadcastEDPacketLo(receivedlo,lambdalo,SumYlo[0],SumZlo[0],arraySize);}
		        else if (millis()%3==2) {_broadcastEDPacketHi(receivedhi,lambdahi,SumYhi[0],SumZhi[0],arraySize);}

                uint8_t avail = _rx->getData(7);
                bool is_upper = (_rx->getData(8)>>7) & 1;
                // Serial << "Data received "<<avail<<endl;
                // delay(5);
                if (is_upper){
                	comm_with_neighbor_hi = true;
                	comm_in_neighbor_hi[nei2index[neighborID-1]] = true;
                	float inSumZhi;float* tmp = _getFloat32FromEDPacket(2,1); inSumZhi = tmp[0];
	                float* inLambdahi = _getFloat16FromEDPacket(8,arraySize);
	                float* inSumYhi = _getFloat32FromEDPacket(21,arraySize);
	                for(uint8_t i = 0; i < arraySize; i++)
	                {
	                    if (((avail>>i) & 1) && !((next_receivedhi>>i) & 1)){
	                        // Serial<<"New upper lambda of node "<<i+1<<endl;
	                        // delay(5);
	                        lambdahi[i] = inLambdahi[i];
	                        Yhi[i] = dual_function(alpha_p,beta_p,max_p,min_p,inLambdahi[i]);
	                        next_receivedhi |= 1<<i;
	                        // Serial<<"High Y value: "<<Yhi[i]<<endl;
	                        // delay(5);
	                    }
	                ...
	                for(uint8_t i = 0; i < arraySize; i++){
	                    if ((receivedhi>>i) & 1) {
	                        newYhi[i] += inSumYhi[i] - SumYhi[1+nei2index[neighborID-1]][i];
	                        SumYhi[1+nei2index[neighborID-1]][i] = inSumYhi[i];
	                    }
	                }

	                newZhi += inSumZhi - SumZhi[1+nei2index[neighborID-1]];
	                SumZhi[1+nei2index[neighborID-1]] = inSumZhi;
                }
                else{
                	comm_with_neighbor_lo = true;
                	comm_in_neighbor_lo[nei2index[neighborID-1]] = true;
                	float inSumZlo;float* tmp = _getFloat32FromEDPacket(2,1); inSumZlo = tmp[0];
					float* inLambdalo = _getFloat16FromEDPacket(8,arraySize);
	                float* inSumYlo = _getFloat32FromEDPacket(21,arraySize);
	                for(uint8_t i = 0; i < arraySize; i++)
	                {
	                    if (((avail>>i) & 1) && !((next_receivedlo>>i) & 1)){
	                        // Serial<<"New lower lambda of node "<<i+1<<endl;
	                        // delay(5);
	                        lambdalo[i] = inLambdalo[i];
	                        Ylo[i] = dual_function(alpha_p,beta_p,max_p,min_p,inLambdalo[i]);                        
	                        next_receivedlo |= 1<<i;
	                ...
	                for(uint8_t i = 0; i < arraySize; i++){
	                    if ((receivedlo>>i) & 1) {
	                        newYlo[i] += inSumYlo[i] - SumYlo[1+nei2index[neighborID-1]][i];
	                        SumYlo[1+nei2index[neighborID-1]][i] = inSumYlo[i];
	                    }
	                }

	                newZlo += inSumZlo - SumZlo[1+nei2index[neighborID-1]];
	                SumZlo[1+nei2index[neighborID-1]] = inSumZlo;
                }
            ...
            // _broadcastEDPacket(received,lambdalo,lambdahi,SumYlo[0],SumYhi[0],SumZ[0],arraySize);
            // if (millis()%2) {_broadcastEDPacketLo(receivedlo,lambdalo,SumYlo[0],SumZlo[0],arraySize);}
            // else _broadcastEDPacketHi(receivedhi,lambdahi,SumYhi[0],SumZhi[0],arraySize);
        ...
        if (comm_with_neighbor_lo) Zlo = Zlo/Dout;      
        if (comm_with_neighbor_hi) Zhi = Zhi/Dout;
        Zlo += newZlo; Zhi += newZhi;
		for(uint8_t i = 0; i < arraySize; i++){
            if ((receivedlo>>i) & 1) {
            	if (comm_with_neighbor_lo) Ylo[i] = Ylo[i]/Dout;      
                Ylo[i] += newYlo[i];
            }
            if ((receivedhi>>i) & 1) {
                if (comm_with_neighbor_hi) Yhi[i] = Yhi[i]/Dout;
                Yhi[i] += newYhi[i];
            }
            ratioslo[i] = Ylo[i]/Zlo;ratioshi[i] = Yhi[i]/Zhi;
        }
        ...
        String str;
        if (k%10==0){
	        // str = "Low lambda values: ";_print1Darray_(str,lambdalo,arraySize,5);
	        // str = "High lambda values: ";_print1Darray_(str,lambdahi,arraySize,5);
	        str = "Low ratio values: ";_print1Darray_(str,ratioslo,arraySize,5);
	        str = "High ratio values: ";_print1Darray_(str,ratioshi,arraySize,5);
	        str = "Y hi running sums: ";_print1Darray_(str,SumYhi[0],arraySize,4);
	        str = "Y lo running sums: ";_print1Darray_(str,SumYlo[0],arraySize,4);
	       	Serial<<"Z hi running sum: "<<SumZhi[0]<<endl; delay(5);
	       	Serial<<"Z lo running sum: "<<SumZlo[0]<<endl; delay(5);
	       	str = "Y hi: ";_print1Darray_(str,Yhi,arraySize,4);
	       	str = "Y lo: ";_print1Darray_(str,Ylo,arraySize,4);
	       	Serial<<"Z hi: "<<Zhi<<endl; delay(5);
	       	Serial<<"Z lo: "<<Zlo<<endl; delay(5);
 ...
 //    Serial<<"Received register: "<<receivedlo<<" "<<receivedhi<<endl;
	// delay(5);
 //    Serial<<"Low ratio value : "<<valueMinPlus[0]<<" "<<ratioMinPlus[0]<<endl;
	// delay(5);
	// Serial<<"High ratio value : "<<valueMinPlus[1]<<" "<<ratioMinPlus[1]<<endl;
...
bool OAgent_OPF::_ScheduleNonLeaderOPF(unsigned long &startTime, uint8_t &iterations){
	...
	if(_waitForNeighborPacket(neighborID,header,true,5000))                        //stays in loop until desired packet received
 	{
...
void OAgent_OPF::_broadcastEDPacketLo(uint8_t receivedlo,float* lambdalo,float* SumYlo,float SumZlo,uint8_t arraySize) {
    ...
    _sender_helper_32bit(SumZlo,&sign_var,&var);   
    payload[2] = sign_var;
    payload[3] = var;
    payload[4] = var >> 8;
    payload[5] = var >> 16;
    payload[6] = var >> 24;
    payload[7] = receivedlo;

    // Serial << "lambda values sent in uint16_t are:";
    // delay(5);
    payload[8] = 0; payload[21] = 0;
    uint16_t var16;
    int i = 8;
    for (int j=0;j<arraySize;j++){
        _sender_helper_16bit(lambdalo[j],&sign_var,&var16);
        payload[8] += sign_var<<j;
        payload[i+1] = var16;
        payload[i+2] = var16 >> 8;
        i+=2;
    }
    i+=1;
    for (int j=0;j<arraySize;j++){
        _sender_helper_32bit(SumYlo[j],&sign_var,&var);
        payload[21] += sign_var<<j;
        payload[i+1] = var;
        payload[i+2] = var >> 8;
        payload[i+3] = var >> 16;
        payload[i+4] = var >> 24;
...
void OAgent_OPF::_broadcastEDPacketHi(uint8_t receivedhi,float* lambdahi,float* SumYhi,float SumZhi,uint8_t arraySize) {
    //if arraysize is greater than 10, split it into two broadcasts
    uint8_t payload[46];
    ...
    uint8_t sign_var; uint32_t var;
    _sender_helper_32bit(SumZhi,&sign_var,&var);   
    payload[2] = sign_var;
    payload[3] = var;
    payload[4] = var >> 8;
    payload[5] = var >> 16;
    payload[6] = var >> 24;
    payload[7] = receivedhi;

    // Serial << "lambda values sent in uint16_t are:";
    // delay(5);
    payload[8] = 1<<7; // Data packets correspond to upper dual variables
    uint16_t var16;
    int i = 8;
    for (int j=0;j<arraySize;j++){
        _sender_helper_16bit(lambdahi[j],&sign_var,&var16);
        payload[8] += sign_var<<j;
        payload[i+1] = var16;
        payload[i+2] = var16 >> 8;
        i+=2;
    ...
    i+=1; payload[21] = 0;
    for (int j=0;j<arraySize;j++){
        _sender_helper_32bit(SumYhi[j],&sign_var,&var);
        payload[21] += sign_var<<j;
        payload[i+1] = var;
        payload[i+2] = var >> 8;
        payload[i+3] = var >> 16;
...
void OAgent_OPF::_sender_helper_16bit(float x,uint8_t* sign_y,uint16_t* y){
        if (x < 0) 
        {
            *y = (uint32_t) (-x*BASE_16);
            *sign_y= 0;
        }
        else
        {
...
float* OAgent_OPF::_getFloat16FromEDPacket(uint8_t pos,uint8_t n_var) {
    float *a = new float[n_var];
    int16_t mag_x; int8_t sign_x; float x;
    int i = pos;
    for (int j=0;j<n_var;j++){
        sign_x = -1 + (((_rx->getData(pos))>>j) & 1)*2;
        mag_x = (int16_t(_rx->getData(i+2)) << 8) + int16_t(_rx->getData(i+1));
...
float* OAgent_OPF::_getFloat32FromEDPacket(uint8_t pos,uint8_t n_var) {
    float *a = new float[n_var];
    int32_t mag_x; int8_t sign_x; float x;
    int i = pos;
    for (int j=0;j<n_var;j++){
        sign_x = -1 + (((_rx->getData(pos))>>j) & 1)*2;
        mag_x = (int32_t(_rx->getData(i+4)) << 24) + (int32_t(_rx->getData(i+3)) << 16) + (int32_t(_rx->getData(i+2)) << 8) + int32_t(_rx->getData(i+1));
        x = (float) (sign_x*mag_x);
        a[j] = x/BASE_32;
        i += 4;
        // Serial<<sign_x<<" "<<a[j]<<" "<<mag_x<<endl;
