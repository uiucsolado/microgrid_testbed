...
float OAgent_OPF::economicDispatchAlgorithm(float alpha_p, float beta_p, float max_p, float min_p, float u, uint16_t iterations, uint16_t period) {
    ...
    return economicDispatch(alpha_p,beta_p,max_p,min_p,u,iterations,period);
...
float OAgent_OPF::economicDispatch(float alpha_p, float beta_p, float max_p, float min_p, float u, uint16_t iterations, uint16_t period) {
    ...
    for(uint8_t k = 0; k < iterations; k++)
    {
        ...
        for(uint8_t i = 0; i < arraySize; i++){
            ...
            SumYlo[0][i]+= Ylo[i]/Dout; SumYhi[0][i]+= Yhi[i]/Dout;
        }
        SumZlo[0]  += Zlo/Dout; Zlo = Zlo/Dout;
        SumZhi[0]  += Zhi/Dout; Zhi = Zhi/Dout;
        start = millis();   // initialize timer
        uint8_t next_receivedhi = receivedhi; uint8_t next_receivedlo = receivedlo;
        while( uint16_t(millis()-start) < period )
        {
            receivedPacket = _waitForNeighborPacket(neighborID,ED_HEADER,true,50);
            ...
            if ( receivedPacket && !(comm_in_neighbor_lo[nei2index[neighborID-1]] || comm_in_neighbor_hi[nei2index[neighborID-1]]) )                                                                         //get values for fp, fq, and lambda that are received from this neighbor
            {
                ...
                if (is_upper){
	                ...
	                Zhi += inSumZhi - SumZhi[1+nei2index[neighborID-1]];
	                ...
	                Zlo += inSumZlo - SumZlo[1+nei2index[neighborID-1]];
            ...
            _broadcastEDPacketLo(receivedlo,lambdalo,SumYlo[0],SumZlo[0],arraySize);
            _broadcastEDPacketHi(receivedhi,lambdahi,SumYhi[0],SumZhi[0],arraySize);
        ...
        for(uint8_t i = 0; i < arraySize; i++){
            if ((receivedlo>>i) & 1) {
                Ylo[i] = newYlo[i] + Ylo[i]/Dout;
            }
            if ((receivedhi>>i) & 1) {
                Yhi[i] = newYhi[i] + Yhi[i]/Dout;
            }
            ratioslo[i] = Ylo[i]/(1e-6+Zlo);ratioshi[i] = Yhi[i]/(1e-6+Zhi);
        }
    	...
    	// Serial<<"High ratio values "<<ratioshi[0]<<" "<<ratioshi[2]<<" "<<ratioshi[5]<<endl;

    }
    float valueMinPlus[2] = {lambdalo[nodeID-1],lambdahi[nodeID-1]};
    float ratioMinPlus[2] = {ratioslo[nodeID-1],ratioshi[nodeID-1]};
    for(uint8_t i = 0; i < arraySize; i++){
