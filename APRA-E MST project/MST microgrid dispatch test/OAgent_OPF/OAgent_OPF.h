...
#define BASE_32                          100000000
#define BASE_16                          100000
...
#define SCHEDULE_OPF_DELAY               5000   // delay before primal dual algorithm starts
...
class OAgent_OPF {
	public:
        ...
        float dual_function(float alpha_p, float beta_p, float max_p, float min_p, float lambda);
	...
	private:
        ...
        void _broadcastEDPacket(uint8_t received,float* lambdalo,float* lambdahi,float* SumYlo,float* SumYhi,float SumZ,uint8_t arraySize);
        void _broadcastEDPacketLo(uint8_t receivedlo,float* lambdalo,float* SumYlo,float SumZlo,uint8_t arraySize);
        void _broadcastEDPacketHi(uint8_t receivedhi,float* lambdahi,float* SumYhi,float SumZhi,uint8_t arraySize);
        ...
        float* _getFloat16FromEDPacket(uint8_t pos,uint8_t n_var);
        float* _getFloat32FromEDPacket(uint8_t pos,uint8_t n_var);
        void _sender_helper_16bit(float x,uint8_t* sign_y,uint16_t* y);
