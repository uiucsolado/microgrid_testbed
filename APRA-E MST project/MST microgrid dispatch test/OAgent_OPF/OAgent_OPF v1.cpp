...
float OAgent_OPF::economicDispatch(float alpha_p, float beta_p, float max_p, float min_p, float u, uint16_t iterations, uint16_t period) {
	...
	float Z = u;
	float * SumZ = new float[1+deg];
	for(uint8_t i = 0; i < 1+deg; i++){ SumZ[i] = 0;}
    ...
    for(uint8_t k = 0; k < iterations; k++)
    {
        ...
        SumZ[0]  += Z/Dout; Z = Z/Dout;
        start = millis();   // initialize timer
        while( uint16_t(millis()-start) < period )
        {
            receivedPacket = _waitForNeighborPacket(neighborID,ED_HEADER,true,150);
            ...
            if ( receivedPacket && (*(neighborStatusP+neighborID-1) == 2) )                                                                         //get values for fp, fq, and lambda that are received from this neighbor
            {
                ...
                *(neighborStatusP+neighborID-1) = 3;                                                            //set neighbor status to 3, i.e., note that node has received from this neighbor in this iteration
                ...
                //get running sum values for Lambda and NU that are received from this neighbor
                float inSumZ;
                float* tmp = _getFloat32FromEDPacket(2,1); inSumZ = tmp[0];
                uint8_t avail = _rx->getData(7);
                Serial << "Data received "<<avail<<endl;
                delay(5);
                float* inLambdalo = _getFloat16FromEDPacket(8,arraySize);
                float* inLambdahi = _getFloat16FromEDPacket(21,arraySize);
                float* inSumYlo = _getFloat32FromEDPacket(34,arraySize);
                float* inSumYhi = _getFloat32FromEDPacket(59,arraySize);
                for(uint8_t i = 0; i < arraySize; i++)
                {
                    if (((avail>>i) & 1) && !((received>>i) & 1)){
                        Serial<<"New lambda of node "<<i+1<<endl;
                        delay(5);
                ...
                Z += inSumZ - SumZ[1+nei2index[neighborID-1]];
                SumZ[1+nei2index[neighborID-1]] = inSumZ;
        ...
        for(uint8_t i = 0; i < arraySize; i++){
            ...
            ratioslo[i] = Ylo[i]/(1e-6+Z);ratioshi[i] = Yhi[i]/(1e-6+Z);
        }
...
void OAgent_OPF::_broadcastEDPacket(uint8_t received,float* lambdalo,float* lambdahi,float* SumYlo,float* SumYhi,float SumZ,uint8_t arraySize) {
    ...
    _sender_helper_32bit(SumZ,&sign_var,&var);   
    payload[2] = sign_var;
    payload[3] = var;
    payload[4] = var >> 8;
    payload[5] = var >> 16;
