...
float OAgent_ED::EconomicDispatch2(float step_size,uint16_t iterations) {
    ...
    float SumYlo[1+deg][num_nodes], SumYhi[1+deg][num_nodes], SumZlo[1+deg][num_nodes], SumZhi[1+deg][num_nodes],SumZlo_prev[1+deg][num_nodes];
    for (int i=0;i<1+deg;i++){
        for (int j=0;j<num_nodes;j++){
        	SumYlo[i][j]=0;SumYhi[i][j]=0;SumZlo[i][j]=0;SumZhi[i][j]=0;SumZlo_prev[i][j]=0;
        }
    }
	for(int i = 0; i < num_nodes; i++){
            ...
            if ( _waitForNeighborPacket(neighborID,ED_HEADER,true,50) )
            {
	            ...
	            else
	            {
	            	if (!comm_neighbor_lo[nei_idx])
	            	{
		                ...
		                for(int i = 0; i < num_nodes; i++){
		                	if ((avail>>i) & 1){
		                        newZlo[i] += inSumZlo[i] - SumZlo[nei_idx][i]; SumZlo_prev[nei_idx][i] = SumZlo[nei_idx][i];
