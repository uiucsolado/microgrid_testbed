...
class OLocalVertex : public OVertex {
    public:
        ...
        inline void setDERparams(float min, float max, float alpha, float beta) { _min = min;_max = max;_alpha = alpha;_beta = beta; }
        ...
        inline int16_t getActiveDemand(int16_t i=0) { return *(_pd+i); }
        ...
        inline void setActiveDemand(int16_t *pd) {_pd = pd; }
	...
	protected:
        ...
        int16_t *_pd; //per-unit active power demand
...
class OLocalReserveVertex : public OLocalVertex {
	public:
		OLocalReserveVertex();
        OLocalReserveVertex(uint32_t aLsb, float min, float max, float alpha, float beta, uint8_t Dout, long base);
        OLocalReserveVertex(XBeeAddress64 a, float min, float max, float alpha, float beta, uint8_t Dout, long base);
	...
	private:	
		...
		float _limitGamma;
		/// Methods
		void _prepareOLocalReserveVertex(uint32_t aLsb, float min, float max, float alpha, float beta, uint8_t Dout, long base);
};
...
class OGraph_ED {
	public:
        /// Constructors
		OGraph_ED();
        OGraph_ED(OLocalVertex * s);
        OGraph_ED(OLocalVertex * s, LinkedList * l);
