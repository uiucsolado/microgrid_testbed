...
OLocalReserveVertex::OLocalReserveVertex(uint32_t aLsb, float min, float max, float alpha, float beta, uint8_t Dout, long base) {
	_prepareOLocalReserveVertex(aLsb,min,max,alpha,beta,Dout,base);
}

OLocalReserveVertex::OLocalReserveVertex(XBeeAddress64 a, float min, float max, float alpha, float beta, uint8_t Dout, long base) {
	_prepareOLocalReserveVertex(a.getLsb(),min,max,alpha,beta,Dout,base);
}
/// End public methods
/// Private methods
void OLocalReserveVertex::_prepareOLocalReserveVertex(uint32_t aLsb, float min, float max, float alpha, float beta, uint8_t Dout, long base) {
    _reserveMin = min;
    _reserveMax = max;
...
OGraph_ED::OGraph_ED(OLocalVertex * s) {
    _n = 1;
    _self = s;
    //linkedlist for online neighbors
    LinkedList _l = LinkedList();
...
OGraph_ED::OGraph_ED(OLocalVertex * s, LinkedList * l) {
    _n = 1;
    _self = s;
    //linkedlist for online neighbors
    _list = l;
...
// Local vertex
//OLocalVertex * OGraph_ED::getLocalVertex() {
//    return _self;
//}

bool OGraph_ED::isLocalVertex(uint32_t aLsb) {
...
//bool OGraph_ED::isLocalVertex(XBeeAddress64 a) {
//	return isLocalVertex(a.getLsb());
//}

// Graph properties
//uint8_t OGraph_ED::getN() {
//    return _n;
//}

//uint8_t OGraph_ED::getNumberNonNeighbors() {
//    return _n - _self->getInDegree() - 1;
//}

// Add and remove vertices from in-neighborhood
bool OGraph_ED::addInNeighbor(XBeeAddress64 a, uint8_t neighborID) {
    return addInNeighbor(a.getLsb(),neighborID);
}

bool OGraph_ED::addInNeighbor(uint32_t aLsb, uint8_t neighborID) {
    return addInNeighbor(aLsb,neighborID,0,0);
}

bool OGraph_ED::addInNeighbor(XBeeAddress64 a, uint8_t neighborID, float r, float x) {
    return addInNeighbor(a.getLsb(),neighborID,r,x);
}

bool OGraph_ED::addInNeighbor(uint32_t aLsb, uint8_t neighborID, float r, float x) {
...
bool OGraph_ED::removeInNeighbor(uint8_t neighborID){
...
bool OGraph_ED::isInNeighbor(XBeeAddress64 a) {
    return isInNeighbor(a.getLsb());
}

bool OGraph_ED::isInNeighbor(uint32_t aLsb) {
    uint8_t i;
    return isInNeighbor(aLsb,i);
}

bool OGraph_ED::isInNeighbor(XBeeAddress64 a, uint8_t &i) {
    return isInNeighbor(a.getLsb(),i);
}

bool OGraph_ED::isInNeighbor(uint32_t aLsb, uint8_t &i) {
    if(_isRemoteVertex(aLsb,i))
        return _remoteVertices[i].isInNeighbor();
    return false;
}

bool OGraph_ED::isInNeighbor(uint32_t aLsb, ORemoteVertex * &v) {
    uint8_t i;
...
bool OGraph_ED::addNonNeighbor(XBeeAddress64 a, uint8_t neighborID) {
    return addNonNeighbor(a.getLsb(),neighborID);
}

bool OGraph_ED::addNonNeighbor(uint32_t aLsb, uint8_t neighborID) {
    return addNonNeighbor(aLsb,neighborID,0,0);
}

bool OGraph_ED::addNonNeighbor(XBeeAddress64 a, uint8_t neighborID, float r, float x) {
    return addNonNeighbor(a.getLsb(),neighborID,r,x);
}

bool OGraph_ED::addNonNeighbor(uint32_t aLsb, uint8_t neighborID, float r, float x) {
    return _addRemoteVertex(aLsb,neighborID,r,x);
}

// Determine if vertex is non-neighbor
bool OGraph_ED::isNonNeighbor(XBeeAddress64 a) {
    return isNonNeighbor(a.getLsb());
}

bool OGraph_ED::isNonNeighbor(uint32_t aLsb) {
    uint8_t i;
    return isNonNeighbor(aLsb,i);
}

bool OGraph_ED::isNonNeighbor(XBeeAddress64 a, uint8_t &i) {
    return isNonNeighbor(a.getLsb(),i);
}

bool OGraph_ED::isNonNeighbor(uint32_t aLsb, uint8_t &i) {
    if(_isRemoteVertex(aLsb,i))
        return !(_remoteVertices[i].isInNeighbor());
    return false;
}
...
void OGraph_ED::clearAllYMinYMax() {
    _self->clearYMinYMax();
	for(uint8_t i = 0; i < NUM_REMOTE_VERTICES; i++)
		_remoteVertices[i].clearYMinYMax();
}

void OGraph_ED::clearAllInStates() {
    _self->clearZIn();
...
void OGraph_ED::clearAllBroadcastStates() {
	_self->clearSigma();
...
OVertex * OGraph_ED::getVertexByAddressLsb(uint32_t aLsb) {
    uint8_t i;
    return getVertexByAddressLsb(aLsb,i);
}

// Methods to pointer to vertex
OVertex * OGraph_ED::getVertexByAddressLsb(uint32_t aLsb, uint8_t &i) {
...
OVertex * OGraph_ED::getVertexByAddress(XBeeAddress64 a) {
	return getVertexByAddressLsb(a.getLsb());
}

OVertex * OGraph_ED::getVertexByUniqueID(uint8_t i) {
	if(i == 0) {
		return dynamic_cast<OVertex*> (_self);
...
ORemoteVertex * OGraph_ED::getRemoteVertex(uint8_t neighborID) {
...
void OGraph_ED::configureLinkedList() {
    _list->updateLinkedList(_self->getStatusP());
}

uint8_t OGraph_ED::_getRemoteVertexIndex(XBeeAddress64 a) {
    return _getRemoteVertexIndex(a.getLsb());
}

uint8_t OGraph_ED::_getRemoteVertexIndex(uint32_t aLsb) {
	for(uint8_t i = 0; i < NUM_REMOTE_VERTICES; i++) {
...
bool OGraph_ED::_isRemoteVertex(uint32_t aLsb) {
    uint8_t i = _getRemoteVertexIndex(aLsb);
    return _isRemoteVertex(aLsb,i);
}

bool OGraph_ED::_isRemoteVertex(uint32_t aLsb, uint8_t &i) {
...
bool OGraph_ED::_addRemoteVertex(uint32_t aLsb, uint8_t neighborID, float r, float x, bool inNeighbor) {
