...
// Added by Stanton T. Cady 06/26/12
void XBeeResponse::setCommon(XBeeResponse * target) {
...
#ifdef DEBUG
    Serial << "getZBRxResponse address 0x" << _HEX((unsigned int) rxResponse) << endl;
#endif
...
//void XBeeResponse::getZBRxResponse(ZBRxResponse * zb, XBeeResponse * rxResponse) {
...
// Added by Stanton T. Cady 03/02/11
// Does everything readPacket() does but returns the time that the packet was received
unsigned long XBee::readPacketTwo(bool returnTime) {
...
// This function is to be deprecated soon but is provided for backward compatibility
unsigned long XBee::readPacketT() {
	return readPacketTwo(true);
}
...
bool XBeeAddress64::operator==(const XBeeAddress64 &other) const {
	if(this->_lsb == other._lsb) {
		if(this->_msb == other._msb) {
			return true;
		}
	}
	return false;
...
/* Added by Stanton T. Cady
 * Performs same function as normal send but can record and return send time and include send time in outgoing packet
 */
unsigned long XBee::sendTwo(XBeeRequest &request, bool sendTime, bool returnTime) {
	...
	if(sendTime) {
		length = request.getFrameDataLength() + 4;
	} else {
		length = request.getFrameDataLength();
	} 

	// send length
	uint8_t msbLen = ((length + 2) >> 8) & 0xff;
	uint8_t lsbLen = (length + 2) & 0xff;
	...
	if(sendTime) { // check if time should be sent
		// break current time into 4 bytes and send while incrementing checksum
		// time will be the last part of the data in the packet
		uint8_t byte1 = currentTime;
		uint8_t byte2 = currentTime >> 8;
		uint8_t byte3 = currentTime >> 16;
		uint8_t byte4 = currentTime >> 24;
		sendByte(byte1,true);
		checksum += byte1;
		sendByte(byte2,true);
		checksum += byte2;
