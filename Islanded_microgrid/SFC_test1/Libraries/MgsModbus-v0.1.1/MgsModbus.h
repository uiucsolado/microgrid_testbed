...
public:
  ...
  // modbus master
  void Build(int fc,uint8_t Ref_highbit,uint8_t Ref_lowbit,uint8_t Count_highbit,uint8_t Count_lowbit,uint8_t Pos_highbit,uint8_t Pos_lowbit,int nodeip);
  void Req(MB_FC FC, word Ref, word Count, word Pos, int nodeip);
  void MbmRun();
...
private: 
  ...
  MB_FC MbsFC;
  MB_FC MbmFC;    //for the master application
};
