...
OLocalVertex s = OLocalVertex(0x4151C48B,0,0.0479*D_base,-1.5*base,0.5*base,4,D_base,4);  //#NODE
//OLocalVertex s = OLocalVertex(0x4151C48B,0,1*D_base,-2.1667*base,0.1667*base,5,D_base); //changed Dout to 1, OLocalVertex(0x404980AE,-0.1*base,0.05*base,-2.1667*base,0.1667*base,1,base); //SN The Xbee address  needs to be changed here right? Also  figure out how to assign the 
...
byte mac[] = {0x90, 0xA2, 0xDA, 0x0E, 0x94, 0xB4 }; //#NODE
IPAddress ip(192, 168, 2, 4); // What are these addresses //#NODE
...
void sendConsensusResults()
{
  ...
  ////////////////////////////////////////////////////////////////
  int node4_ip = 64; //part of ip address for node 4 on the HIL side  //#NODE
  Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,0,2,1,node4_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos - arduino, int nodeip) //#NODE
  Mb.MbmRun();
...
{
  int node4_ip = 64; //part of ip address for node 4 on the HIL side //#NODE
  Mb.Req(MB_FC_READ_INPUT_REGISTER,0,1,0,node4_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos -arduino, int nodeip) //#NODE
  Mb.MbmRun();
}
