...
OLocalVertex s = OLocalVertex(0x4174F186,0,0.679*D_base,-1.5*base,0.5*base,4,D_base,2);  //#NODE
//OLocalVertex s = OLocalVertex(0x4174F186,0,1*D_base,-2.1667*base,0.1667*base,5,D_base); //changed Dout to 1, OLocalVertex(0x404980AE,-0.1*base,0.05*base,-2.1667*base,0.1667*base,1,base); //SN The Xbee address  needs to be changed here right? Also  figure out how to assign the 
OGraph_SFC g = OGraph_SFC(&s);
OAgent_SFC a = OAgent_SFC(&xbee,&rx,&g,false,true); // argument rx?
...
byte mac[] = {0x90, 0xA2, 0xDA, 0x0E, 0x94, 0xB2 }; //#NODE
IPAddress ip(192, 168, 2, 2); // What are these addresses //#NODE
...
void sendConsensusResults()
{
  ...
  ////////////////////////////////////////////////////////////////
  int node2_ip = 62; //part of ip address for node 2 on the HIL side  //#NODE
  Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,0,2,1,node2_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos - arduino, int nodeip) //#NODE
  Mb.MbmRun();
...
{
  int node2_ip = 62; //part of ip address for node 2 on the HIL side
  Mb.Req(MB_FC_READ_INPUT_REGISTER,0,1,0,node2_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos -arduino, int nodeip) //#NODE
  Mb.MbmRun();
}
