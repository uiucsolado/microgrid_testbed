...
void sendConsensusResults()
{
  ...
  Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,0,2,1,node1_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos - arduino, int nodeip) //#NODE
  Mb.MbmRun();
...
void receiveTyphoonData()
{
  ...
  Mb.Req(MB_FC_READ_INPUT_REGISTER,0,1,0,node1_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos -arduino, int nodeip) //#NODE
  Mb.MbmRun();
}
