...
OLocalVertex s = OLocalVertex(0x4151C692,0,0.95*D_base,-1.5*base,0.5*base,4,D_base,3);  //#NODE
...
void sendConsensusResults()
{
  ...
  Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,0,2,1,node3_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos - arduino, int nodeip) //#NODE
  Mb.MbmRun();
...
void receiveTyphoonData()
{
  ...
  Mb.Req(MB_FC_READ_INPUT_REGISTER,0,1,0,node3_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos -arduino, int nodeip) //#NODE
  Mb.MbmRun();
}
