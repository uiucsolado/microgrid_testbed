...
OLocalVertex s = OLocalVertex(0x415786D3,10,0,1,1,1,1,10);  //#NODE
LinkedList l = LinkedList();  //#NODE
OGraph_PDRC g = OGraph_PDRC(&s,&l);
OAgent_PDRC a = OAgent_PDRC(&xbee,&rx,&g,false,true); // argument rx?
...
byte mac[] = {0x90, 0xA2, 0xDA, 0x0E, 0x94, 0xBA };   //  #NODE
IPAddress ip(192, 168, 2, 10);   // #NODE
...
//Define the Neighboring nodes
  ...
  g.addInNeighbor(0x415786A9,12,0.03,0.055); // node 12
  //g.addInNeighbor(0x4157847B,13,0,0); // node 13
  g.addInNeighbor(0x415DB664,14,0.03,0.055); // node 14
  ...
  s.setActivePowerWeight(1.2);
  s.setReactivePowerWeight(1.2);
...
void sendConsensusResults()
{
  ...
  ////////////////////////////////////////////////////////////////
  int node10_ip = 70; //part of ip address for node 1 on the HIL side    #NODE
  Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,0,2,0,node10_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos - arduino, int nodeip)   #NODE
  Mb.MbmRun();
...
{
  int node10_ip = 70; //part of ip address for node 1 on the HIL side   #NODE
  Mb.Req(MB_FC_READ_INPUT_REGISTER,0,2,0,node10_ip); //(MB_FC FC, word Ref, word Count, word Pos, int nodeip)   #NODE
  Mb.MbmRun();
}
