...
OLocalVertex s = OLocalVertex(0x415DB664,14,0,1,1,1,1,10);  //#NODE
...
byte mac[] = {0x90, 0xA2, 0xDA, 0x0E, 0x94, 0xBE};   //  #NODE
IPAddress ip(192, 168, 2, 14);   // #NODE
...
void loop() {
  ...
  else 
  {
    if(a.isSynced())
    {
      ...
      ecoDispatch = a.economicDispatchAlgorithm(false,0.1,1000);
...
void sendConsensusResults()
{
  ...
  ////////////////////////////////////////////////////////////////
  int node14_ip = 74; //part of ip address for node 1 on the HIL side    #NODE
  Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,0,2,0,node14_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos - arduino, int nodeip)   #NODE
  Mb.MbmRun();
...
{
  int node14_ip = 74; //part of ip address for node 1 on the HIL side   #NODE
  Mb.Req(MB_FC_READ_INPUT_REGISTER,0,2,0,node14_ip); //(MB_FC FC, word Ref, word Count, word Pos, int nodeip)   #NODE
  Mb.MbmRun();
}
