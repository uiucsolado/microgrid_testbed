...
OLocalVertex s = OLocalVertex(0x4157847B,13,0,1,1,1,1,10);  //#NODE
...
byte mac[] = {0x90, 0xA2, 0xDA, 0x0E, 0x94, 0xBD};   //  #NODE
IPAddress ip(192, 168, 2, 13);   // #NODE
...
void sendConsensusResults()
{
  ...
  ////////////////////////////////////////////////////////////////
  int node13_ip = 73; //part of ip address for node 1 on the HIL side    #NODE
  Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,0,2,0,node13_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos - arduino, int nodeip)   #NODE
  Mb.MbmRun();
...
{
  int node13_ip = 73; //part of ip address for node 1 on the HIL side   #NODE
  Mb.Req(MB_FC_READ_INPUT_REGISTER,0,2,0,node13_ip); //(MB_FC FC, word Ref, word Count, word Pos, int nodeip)   #NODE
  Mb.MbmRun();
}
