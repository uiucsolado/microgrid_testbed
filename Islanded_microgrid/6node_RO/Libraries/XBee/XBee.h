...
public:
	 ...
	 * to populate response
	 */
	void getZBRxResponse(XBeeResponse &rxResponse);
	void getZBRxResponse(XBeeResponse * rxResponse);
	//void getZBRxResponse(ZBRxResponse * zb, XBeeResponse * rxResponse) ;
	/**
...
public:
	...
	bool operator==(const XBeeAddress64 &other) const;
	bool operator!=(const XBeeAddress64 &other) const;
private:
	uint32_t _msb;
	uint32_t _lsb;
...
public:
    ...
    //void startSoftSerial(NewSoftSerial serial, long baud);
	/**
	 ...
	 * Same as readPacket() but returns the time the packet was received as an unsigned long
	 */
    unsigned long readPacketTwo(bool returnTime = false);
	unsigned long readPacketT();
	/**
	...
	/**
	 * Added by Stanton T. Cady 03/02/11
	 * Same as send above but returns the time the packet was sent and can send the transmit time as last 4 bytes of payload
	 */
	unsigned long sendTwo(XBeeRequest &request, bool sendTime = false, bool returnTime = false);
