...
OLocalVertex s = OLocalVertex(0x4174F186,0,0.679*D_base,-1.5*base,0.5*base,2,D_base,2);  //#NODE
...
void sendConsensusResults()
{
  ...
  Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,0,4,0,node2_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos - arduino, int nodeip) //#NODE
  Mb.MbmRun();
...
void receiveTyphoonData()
{
  ...
  Mb.Req(MB_FC_READ_INPUT_REGISTER,0,4,0,node2_ip); //(MB_FC FC, word Ref - typhoon, word Count, word Pos -arduino, int nodeip) //#NODE
  Mb.MbmRun();
}
