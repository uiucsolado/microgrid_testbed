...
// For Arduino 1.0
EthernetServer MbServer(MB_PORT);         //server object is established with port number 502
...
void MgsModbus::Build(int fc,uint8_t Ref_highbit,uint8_t Ref_lowbit,uint8_t Count_highbit,uint8_t Count_lowbit,uint8_t Pos_highbit,uint8_t Pos_lowbit,int nodeip)
{
  MbmFC = SetFC(fc);
  word Ref = word(Ref_highbit,Ref_lowbit);
  word Count = word(Count_highbit,Count_lowbit);
  word Pos = word(Pos_highbit,Pos_lowbit);
  Req(MbmFC, Ref, Count, Pos,nodeip);

}


void MgsModbus::Req(MB_FC FC, word Ref, word Count, word Pos, int nodeip)
{
  MbmFC = FC;
  byte ServerIp[] = {192,168,2,nodeip};                 //server ip address differs for each node, 41 - Node1, 42 - Node2 ......
  ...
  if(MbmFC == MB_FC_WRITE_MULTIPLE_REGISTERS) {
    ...
    MbmByteArray[10] = highByte(Count);               //After the start address, we first store the number of registers we are writing to
    MbmByteArray[11] = lowByte(Count);                
    MbmByteArray[12] = (Count*2);                     //indicates the length of data that goes into the registers 
    MbmByteArray[4] = highByte(MbmByteArray[12] + 7); // Lenght high byte
    MbmByteArray[5] = lowByte(MbmByteArray[12] + 7); // Lenght low byte;
    for (int i=0; i<Count;i++) {
...
{  
  //****************** Read from socket ****************
  EthernetClient client = MbServer.available();       //if server is available it returns a client object
  if(client.available())
  {
    delay(10);
    int i = 0;
    ...
    {
      MbsByteArray[i] = client.read();                       //8bits per index (maximum of 260*8 bits can be sent)
      i++;
    }
    MbsFC = SetFC(MbsByteArray[7]);  //Byte 7 of request is Function Code (FC)
  }
  ...
  if(MbsFC == MB_FC_READ_COILS || MbsFC == MB_FC_READ_DISCRETE_INPUT) {
    Start = word(MbsByteArray[8],MbsByteArray[9]);                          //concatenates the two bytes which defines the starting address
    CoilDataLength = word(MbsByteArray[10],MbsByteArray[11]);               //gives number of bits of coil data?
    ByteDataLength = CoilDataLength / 8;                                    //gets that in bytes
    if(ByteDataLength * 8 < CoilDataLength) ByteDataLength++;      
    CoilDataLength = ByteDataLength * 8;                                     //number of bits needs to be a multiple of 8
